...
class MonitoringSetup:
    ...
    def _create_cloudwatch_alarms(self, topic_arn: str) -> List[str]:
        ...
        alarm_configs = [
            ...
            AlarmConfiguration(
                ...
                threshold=2.0,
                comparison_operator="GreaterThanThreshold",
                dimensions={"LoadBalancer": f"app/{self.alb_name}"},
                alarm_actions=[topic_arn],
            ),
            AlarmConfiguration(
    ...
    def _create_dashboard(self) -> str:
        ...
        dashboard_body = (
            dashboard_body.replace("t-developer-service-production", self.service_name)
            .replace("t-developer-cluster-production", self.cluster_name)
            .replace("t-developer-alb-production", self.alb_name)
            .replace(
...
def main():
    ...
    monitor = MonitoringSetup(environment=args.environment, region=args.region)
    ...
    try:
        ...
        print(
            f"https://{args.region}.console.aws.amazon.com/cloudwatch/home?region={args.region}#dashboards:name={results.get('dashboard', '')}"
        )
        print("=" * 60)
