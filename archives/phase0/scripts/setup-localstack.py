...
config = Config(region_name='us-east-1', retries={'max_attempts': 10, 'mode': 'standard'})

def create_s3_buckets():
    """S3 버킷 생성"""
    s3 = boto3.client('s3', endpoint_url=LOCALSTACK_URL, config=config)
    
    buckets = ['t-developer-artifacts', 't-developer-components', 't-developer-templates', 't-developer-test-data']
...
def setup_cloudwatch():
    ...
    log_groups = ['/aws/lambda/t-developer-agents', '/aws/ecs/t-developer-api', '/t-developer/application']
