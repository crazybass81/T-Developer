...
class PlannerAgent(BaseAgent):
    ...
    async def execute(self, input: AgentInput) -> AgentOutput:
        """Create execution plan based on research and analysis.

        Args:
            input: Agent input with research and analysis data

        Returns:
            Execution plan
        """
        try:
            # Get data from SharedContextStore via payload
            external_research = input.payload.get("external_research", {})
            internal_analysis = input.payload.get("internal_analysis", {})
            focus_areas = input.payload.get("focus_areas", [])

            # Determine goal from research and analysis
            improvements = internal_analysis.get("improvements", [])
            best_practices = external_research.get("best_practices", [])

            if improvements:
                goal = f"Improve code quality by addressing {len(improvements)} identified issues"
            else:
                goal = input.payload.get("goal", "Improve code quality based on analysis")
            ...
            # Decompose goal into tasks based on actual improvements needed
            tasks = self._create_tasks_from_analysis(improvements, best_practices, focus_areas)
            if not tasks:
                # Fallback to generic decomposition
                tasks = self.decomposer.decompose(
                    goal, research_data, self.config.max_hours_per_task
                )
            ...
            # Store improvement plan in context store
            ...
            await self.context_store.store_improvement_plan(
                ...
                priorities=[t.id for t in sorted_tasks],
                dependencies={t.id: t.dependencies for t in sorted_tasks if t.dependencies},
                evolution_id=evolution_id,
            )
            self.logger.info(
                f"Stored improvement plan in context store for evolution {evolution_id or 'current'}"
            )

            # Store tasks in metrics for easier access by RefactorAgent
            metrics_output = {
                "tasks_created": len(sorted_tasks),
                "total_hours": total_hours,
                "milestones": len(milestones),
                "critical_path_length": len(plan.get_critical_path()),
                "tasks": [t.to_dict() for t in sorted_tasks],  # Include tasks for RefactorAgent
                "priority_order": [t.id for t in sorted_tasks],  # Task execution order
                "estimated_impact": self._calculate_impact_score(sorted_tasks, improvements),
            }
...
Return as JSON with keys: missing_tasks, time_adjustments, ordering_changes, risks"""
    ...
    def _create_tasks_from_analysis(
        self, improvements: list[dict], best_practices: list[str], focus_areas: list[str]
    ) -> list[Task]:
        """Create tasks from code analysis improvements.

        Args:
            improvements: List of improvements from CodeAnalysisAgent
            best_practices: Best practices from ResearchAgent
        ...
        for imp in improvements:
            task_type = imp.get("type", "unknown")
            location = imp.get("location", "unknown")
            priority = imp.get("priority", "medium")

            # Map improvement type to task
            task = Task(
                id=f"task-{uuid.uuid4().hex[:8]}",
                name=f"Fix {task_type} in {location}",
                description=f"Address {task_type} issue: {imp.get('description', 'Improve code quality')}",
                estimated_hours=self._estimate_task_hours(task_type),
                dependencies=[],
                priority=priority,
                category=self._map_type_to_category(task_type),
            )
            tasks.append(task)

        # Add tasks for best practices if not covered
        for practice in best_practices[:3]:  # Limit to top 3
            if not any(practice.lower() in t.description.lower() for t in tasks):
                task = Task(
                    id=f"task-{uuid.uuid4().hex[:8]}",
                    name=f"Apply: {practice[:50]}",
                    description=f"Implement best practice: {practice}",
                    estimated_hours=2.0,
                    dependencies=[],
                    priority="medium",
                    category="refactor",
    ...
    def _map_type_to_category(self, task_type: str) -> str:
        """Map improvement type to task category."""
        mapping = {
        ...
        return mapping.get(task_type, "refactor")

    def _calculate_impact_score(self, tasks: list[Task], improvements: list) -> float:
        """Calculate estimated impact score of the plan."""
        if not tasks:
            return 0.0

        # Weight by priority and category
        ...
        for task in tasks:
            ...
            category_weight = category_weights.get(task.category, 0.5)
            total_score += priority_weight * category_weight

        # Normalize to 0-1 range
        return min(1.0, total_score / len(tasks))
    ...
    def get_capabilities(self) -> dict[str, Any]:
        ...
        return {
            ...
            "features": [
                ...
                "context_aware_planning",  # Added context awareness
            ],
