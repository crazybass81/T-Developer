...
class SharedDocumentContext:
    ...
    def add_document(self, agent_name: str, document: Dict[str, Any], document_type: str = "analysis") -> None:
        ...
        logger.info(f"Document added by {agent_name} (type: {document_type}) to loop {self.current_loop_number}")
    ...
    def _summarize_content(self, content: Any, max_length: int = 500) -> str:
        ...
        if isinstance(content, str):
            ...
            # 주요 키만 추출
            summary = {k: v for k, v in list(content.items())[:5]}
            return str(summary)
        else:
