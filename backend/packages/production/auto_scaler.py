...
class MetricThreshold:
    ...
    def is_scale_up_triggered(self, values: list[float], now: datetime) -> bool:
    ...
    def is_scale_down_triggered(self, values: list[float], now: datetime) -> bool:
...
class PredictionModel:
    ...
    def predict(
        self, historical_data: list[float], future_timestamps: list[datetime]
    ) -> list[float]:
...
class AutoScaler:
    ...
    def __init__(self, config: dict[str, Any] = None) -> None:
        ...
        self._prediction_models: dict[str, dict[ResourceMetric, PredictionModel]] = defaultdict(
            dict
        )
        ...
        if target.target_id not in self._prediction_models:
            ...
            for metric in [
                ...
                self._prediction_models[target.target_id][metric] = PredictionModel(
                    model_type="linear_trend", prediction_horizon_minutes=30
                )
    ...
    def _get_recent_metrics(
        self, target_id: str, minutes: int = 10
    ) -> dict[ResourceMetric, list[dict]]:
    ...
    def _calculate_scale_up_count(
        self, target: ScalingTarget, threshold: MetricThreshold, metric_values: list[float]
    ) -> int:
    ...
    def _calculate_scale_down_count(
        self, target: ScalingTarget, threshold: MetricThreshold, metric_values: list[float]
    ) -> int:
    ...
    async def _execute_scaling_action(
        self, target: ScalingTarget, decision: dict[str, Any]
    ) -> None:
            ...
            for metric_type, model in self._prediction_models.get(target_id, {}).items():
                try:
                    ...
                    historical_data = self._get_historical_metric_values(
                        target_id, metric_type, hours=24
                    )
                    ...
                    self.logger.error(
                        f"Failed to update prediction model for {target_id}/{metric_type}: {e}"
                    )

    def _get_historical_metric_values(
        self, target_id: str, metric_type: ResourceMetric, hours: int = 24
    ) -> list[float]:
        ...
        return [m["value"] for m in metric_history if m["timestamp"] >= cutoff_time]
    ...
    async def _make_predictive_scaling_decision(self, target: ScalingTarget) -> None:
        ...
        cpu_threshold = next(
            (t for t in target.thresholds if t.metric == ResourceMetric.CPU_UTILIZATION), None
        )
        ...
        if cpu_threshold:
            ...
            if max_predicted > cpu_threshold.scale_up_threshold and target.can_scale_up():
                # Predictive scale up
                new_count = self._calculate_predictive_scale_count(
                    target, max_predicted, cpu_threshold
                )
    ...
    def _calculate_predictive_scale_count(
        self, target: ScalingTarget, predicted_value: float, threshold: MetricThreshold
    ) -> int:
    ...
    async def _evaluate_target_cost_optimization(self, target: ScalingTarget) -> None:
        ...
        if (
            avg_cpu < 20
            and max_cpu < 40
            and target.can_scale_down()
            and not target.is_in_cooldown(cooldown_seconds=1800)
        ):  # 30 min cooldown for cost optimization
    ...
    async def get_scaling_status(self) -> dict[str, Any]:
        ...
        recent_actions = [
            a
            for a in self._scaling_actions
        ...
        return {
            ...
            "recent_actions": {
                ...
                "scale_down": sum(
                    1 for a in recent_actions if a.direction == ScalingDirection.DOWN
                ),
                "success_rate": statistics.mean([a.success for a in recent_actions])
                if recent_actions
                else 1.0,
            },
            "cost": {
    ...
    def get_target_info(self, target_id: str) -> Optional[dict[str, Any]]:
        ...
        memory_values = [
            m["value"] for m in recent_metrics.get(ResourceMetric.MEMORY_UTILIZATION, [])
        ]
