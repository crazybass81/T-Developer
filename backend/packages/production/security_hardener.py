...
class SecurityHardener:
    ...
    async def _load_default_rules(self) -> None:
        ...
        self.logger.info(
            f"Loaded {len(self._waf_rules)} WAF rules and {len(self._rate_limit_rules)} rate limit rules"
        )
    ...
    async def _load_threat_intelligence(self) -> None:
        ...
        self._threat_intelligence["malicious_ips"].update(
            ["198.51.100.100", "203.0.113.50"]  # Example malicious IP  # Example botnet IP
        )

        # Known malicious domains
        self._threat_intelligence["malicious_domains"].update(
            ["malicious-site.example.com", "phishing-domain.test"]
        )
    ...
    async def _check_rate_limits(self, request_data: dict[str, Any]) -> dict[str, Any]:
        ...
        for rule in self._rate_limit_rules.values():
            ...
            cache_entry = self._rate_limit_cache.get(
                cache_key, {"count": 0, "window_start": now, "burst_used": 0}
            )

            # Check if we're in a new window
            if now - cache_entry["window_start"] >= rule.window_seconds:
                cache_entry = {"count": 0, "window_start": now, "burst_used": 0}
    ...
    async def _log_security_event(
        self, request_data: dict[str, Any], threats: list[dict[str, Any]], blocked: bool
    ) -> None:
        ...
        for threat in threats:
            ...
            elif threat_level.value == "medium" and severity not in [
    ...
    async def start_security_scan(
        self, scan_type: ScanType, target: str, config: dict[str, Any] = None
    ) -> ScanResult:
        ...
        scan_result = ScanResult(
            scan_id=scan_id, scan_type=scan_type, target=target, started_at=datetime.utcnow()
        )
    ...
    async def _execute_scan(self, scan_result: ScanResult, config: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Completed scan {scan_result.scan_id}: {len(scan_result.findings)} findings"
            )
    ...
    async def get_security_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "scans": {
                ...
                "completed_today": len(
                    [s for s in completed_scans if s.completed_at and s.completed_at >= last_day]
                ),
                "avg_risk_score": statistics.mean([s.risk_score for s in completed_scans])
                if completed_scans
                else 0,
            },
            "threat_intelligence": {
    ...
    def get_recent_events(
        self, hours: int = 24, severity: Optional[ThreatLevel] = None
    ) -> list[dict[str, Any]]:
        ...
        events = [e for e in self._security_events if e.timestamp >= cutoff_time]
