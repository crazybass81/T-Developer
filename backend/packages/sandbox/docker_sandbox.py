...
class DockerSandbox:
    ...
    async def execute(
        ...
        try:
            ...
            try:
                exit_code, output = await asyncio.wait_for(
                    asyncio.get_event_loop().run_in_executor(
                        ...
                        lambda: self._container.exec_run(
                            command, workdir=working_dir or self.config.working_dir
                        ),
                    ),
                    timeout=timeout,
                )
                ...
                return SandboxResult(
                    exit_code=-1, stdout="", stderr="Command timed out", error="Command timed out"
                )

        except Exception as e:
            logger.error(f"Sandbox execution failed: {e}")
            return SandboxResult(exit_code=-1, stdout="", stderr=str(e), error=str(e))
    ...
    async def execute_python(self, script: str, timeout: Optional[int] = None) -> SandboxResult:
        ...
        try:
            # Execute script
            result = await self.execute(f"python {script_path}", timeout=timeout)
            return result
        finally:
            # Cleanup temp file
            try:
    ...
    async def execute_with_files(
        self, command: str, files: dict[str, str], timeout: Optional[int] = None
    ) -> SandboxResult:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = await self.execute(command, timeout=timeout, working_dir=tmpdir)
