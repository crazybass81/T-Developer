...
@dataclass
class Pattern:
    """Recognized pattern in code or behavior."""
    
    pattern_id: str
    pattern_type: str  # code, behavior, performance
    description: str
    ...
    def add_example(self, example: Dict[str, Any], success: bool) -> None:
...
@dataclass
class Experience:
    """Single experience from task execution."""
...
class Memory:
    ...
    def apply_decay(self, time_delta: timedelta) -> None:
        ...
        decay_rate = self.metadata.get("decay_rate", 0.1)
        hours = time_delta.total_seconds() / 3600
        importance = self.get_importance()
        new_importance = max(0.0, importance * (1 - decay_rate * hours))
        self.metadata["importance"] = new_importance
...
class MemoryCurator:
    ...
    def __init__(
        ...
        self.episodic_memory: Deque[Experience] = deque(maxlen=10000)
