...
class MetricsCollector:
    ...
    def _format_dimensions(self, dimensions: Optional[Dict[str, str]] = None) -> List[Dict[str, str]]:
        """차원 포맷팅"""
        formatted = [
    ...
    def gauge(self, metric_name: str, value: float, dimensions: Optional[Dict[str, str]] = None):
        """게이지 메트릭"""
        self.put_metric(
            metric_name=metric_name,
            value=value,
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                metrics.count(f"{metric_name}_error", dimensions={'function': func.__name__, 'error': type(e).__name__})
                raise
        return wrapper
    return decorator