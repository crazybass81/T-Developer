...
class AudioVideoProcessor:
    ...
    async def extract_key_frames(
        self, video_buffer: bytes, frame_count: int = 3
    ) -> List[Dict[str, Any]]:
    ...
    async def _ffmpeg_extract_frames(
        self, video_buffer: bytes, frame_count: int
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(frame_count):
                ...
                # 프레임 추출
                out, _ = (
                    ffmpeg.input(tmp_path, ss=timestamp)
                ...
                frame_array = np.frombuffer(out, np.uint8).reshape(
                    [int(video_info["height"]), int(video_info["width"]), 3]
                )
