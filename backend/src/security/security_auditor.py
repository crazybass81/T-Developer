...
class SecurityAuditor:
    ...
    def __init__(self):
        ...
        self.compliance_checks = {
            ...
            "pci_dss": ["hardcoded_secrets", "insecure_crypto", "path_traversal"],
            "hipaa": ["hardcoded_secrets", "insecure_crypto"],
            "gdpr": ["hardcoded_secrets", "path_traversal"],
        }
    ...
    async def _scan_javascript_file(
        ...
        dangerous_patterns = [
            ...
            (
                ...
                "critical",
                "CWE-94",
                "Dynamic code execution with Function()",
            ),
            (
                r'setTimeout\s*\(\s*["\'].*["\']',
                "medium",
                "CWE-94",
                "Code injection via setTimeout string",
            ),
            (
                r'setInterval\s*\(\s*["\'].*["\']',
                "medium",
                "CWE-94",
                "Code injection via setInterval string",
            ),
        ]
            ...
            if result.returncode == 0:
                ...
                for result_item in semgrep_output.get("results", []):
                    vuln = SecurityVulnerability(
                        ...
                        .get("metadata", {})
                        .get("cwe", "Unknown"),
                        recommendation=result_item.get("extra", {}).get(
                            "fix", "Review Semgrep documentation"
                        ),
                        confidence="high",
                    )
                    vulnerabilities.append(vuln)
    ...
    def _generate_recommendations(self, vulnerabilities: List[SecurityVulnerability]) -> List[str]:
        ...
        if "hardcoded_secrets" in critical_categories:
            recommendations.append(
                "CRITICAL: Remove hardcoded secrets and use environment variables"
            )
    ...
    def _get_recommendation(self, category: str, rule: Dict[str, str]) -> str:
        ...
        return recommendations.get(
            category, "Review security best practices for this vulnerability type"
        )
