...
class RateLimiter:
    ...
    def _fallback_rate_limit(
        ...
        # Remove old entries
        self._memory_limits[key] = [
            t for t in self._memory_limits[key]
            if t > now - window
        ]
...
class IPBlocker:
    ...
    def __init__(self, redis_client: Optional[redis.Redis] = None):
        ...
        self.thresholds = {
            '401': {'count': 10, 'window': 300, 'block_duration': 3600},  # 10 unauthorized in 5 min = 1 hour block
            '403': {'count': 10, 'window': 300, 'block_duration': 3600},  # 10 forbidden in 5 min = 1 hour block
            '429': {'count': 5, 'window': 60, 'block_duration': 1800},  # 5 rate limits in 1 min = 30 min block
        }
    ...
    async def track_suspicious_activity(
        ...
        try:
            ...
            if count >= threshold['count']:
                await self.block_ip(
                    ip,
                    f"Too many {status_str} responses",
