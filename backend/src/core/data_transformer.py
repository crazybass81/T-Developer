...
from src.core.interfaces import (
    AgentInput, AgentResult, PipelineContext, ProcessingStatus
)
from src.core.agent_models import (
    NLInputResult, UISelectionResult, ParserResult,
    ComponentDecisionResult, MatchRateResult, SearchResult,
    GenerationResult, AssemblyResult, DownloadResult
)
...
class DataTransformer:
    ...
    @staticmethod
    def nl_input_to_ui_selection(nl_result: NLInputResult, 
                                 context: PipelineContext) -> AgentInput[Dict]:
    ...
    @staticmethod
    def ui_selection_to_parser(ui_result: UISelectionResult,
        ...
        return AgentInput(
            data={
                ...
                "state_management": ui_result.state_management.name if ui_result.state_management else None,
    ...
    @staticmethod
    def parser_to_component_decision(parser_result: ParserResult,
        ...
        return AgentInput(
            data={
                ...
                "api_endpoints": [DataTransformer.serialize(ep) for ep in parser_result.api_endpoints],
    ...
    @staticmethod
    def component_to_match_rate(component_result: ComponentDecisionResult,
    ...
    @staticmethod
    def match_rate_to_search(match_result: MatchRateResult,
        ...
        return AgentInput(
            data={
                ...
                "best_match_template": match_result.best_match.template_id if match_result.best_match else None
            },
            context=context,
            previous_results=[]
        )
    ...
    @staticmethod
    def search_to_generation(search_result: SearchResult,
                           component_result: ComponentDecisionResult,
                           parser_result: ParserResult,
        ...
        return AgentInput(
            data={
                ...
                "code_snippets": [DataTransformer.serialize(s) for s in search_result.code_snippets[:5]],
                "template": match_result.best_match.template_id if match_result.best_match else None,
                "api_endpoints": [DataTransformer.serialize(ep) for ep in parser_result.api_endpoints],
                "database_schemas": [DataTransformer.serialize(s) for s in parser_result.database_schemas]
            },
            context=context,
            previous_results=[]
        )
    ...
    @staticmethod
    def generation_to_assembly(generation_result: GenerationResult,
                              parser_result: ParserResult,
    ...
    @staticmethod
    def assembly_to_download(assembly_result: AssemblyResult,
        ...
        return AgentInput(
            data={
                ...
                "tests_passed": assembly_result.test_result.passed_tests if assembly_result.test_result else 0,
    ...
    def extract_final_output(results: List[AgentResult]) -> Optional[Dict[str, Any]]:
        ...
        return {
            "success": True,
            "download_url": download_result.package_url if hasattr(download_result, 'package_url') else None,
            "package_size_mb": download_result.package_size_mb if hasattr(download_result, 'package_size_mb') else None,
            "project_id": download_result.metadata.project_id if hasattr(download_result, 'metadata') else None,
            "documentation_url": download_result.documentation_url if hasattr(download_result, 'documentation_url') else None
        }
...
class DataValidator:
    ...
    def validate_nl_input(data: Dict[str, Any]) -> List[str]:
        ...
        if "complexity" in data and data["complexity"] not in ["simple", "medium", "complex"]:
