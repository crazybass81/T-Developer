    ...
    def to_sqs_format(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "MessageAttributes": {
                ...
                "queue_type": {
                    "DataType": "String",
                    "StringValue": self.queue_type.value,
                },
                "priority": {
                    "DataType": "Number",
                    "StringValue": str(self.priority.value),
                },
                "correlation_id": {
                    "DataType": "String",
                    "StringValue": self.correlation_id or "",
                },
                "timestamp": {
                    "DataType": "String",
                    "StringValue": self.timestamp.isoformat(),
                },
                "retry_count": {
                    "DataType": "Number",
                    "StringValue": str(self.retry_count),
                },
            },
        }
        ...
        try:
            if self.use_sqs and self._sqs_client and self.queue_url:
                ...
                sqs_messages = self._sqs_client.receive_messages(
                    self.queue_url, max_messages=max_messages, wait_time=wait_time
                )
                ...
                while len(messages) < max_messages and datetime.utcnow() < deadline:
                    try:
                        ...
                        priority, timestamp, message = await asyncio.wait_for(
                            self._local_queue.get(), timeout=min(remaining_time, 1.0)
                        )
            ...
            if self.use_sqs and self._sqs_client and self.queue_url:
                ...
                if hasattr(message, "_sqs_receipt_handle"):
                    return self._sqs_client.delete_message(
                        self.queue_url, message._sqs_receipt_handle
                    )
...
class MessageQueueManager:
    ...
    def create_queue(
        self, queue_name: str, queue_type: QueueType, queue_url: Optional[str] = None
    ) -> MessageQueue:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "queue_stats": [queue.get_stats() for queue in self._queues.values()],
        }
