...
class AgentModel(Base):
    ...
    ai_quality_score = Column(Float, CheckConstraint('ai_quality_score >= 0 AND ai_quality_score <= 1'))
    ai_analysis_timestamp = Column(DateTime(timezone=True))
    ai_model_used = Column(String(100))
    ai_confidence_score = Column(Float, CheckConstraint('ai_confidence_score >= 0 AND ai_confidence_score <= 1'))
...
class AgentVersionModel(Base):
    ...
    deployment_status = Column(String(50), default='draft')  # draft, testing, production, deprecated
...
class AgentExecutionModel(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    agent_id = Column(String(100), ForeignKey('agents.agents.agent_id'), nullable=False)
    execution_id = Column(String(100), unique=True, nullable=False, default=lambda: str(uuid.uuid4()))
...
class AgentEvolutionModel(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    evolution_id = Column(String(100), unique=True, nullable=False, default=lambda: str(uuid.uuid4()))
    agent_id = Column(String(100), ForeignKey('agents.agents.agent_id'), nullable=False)
...
class WorkflowModel(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    workflow_id = Column(String(100), unique=True, nullable=False, default=lambda: str(uuid.uuid4()))
    name = Column(String(100), nullable=False)
