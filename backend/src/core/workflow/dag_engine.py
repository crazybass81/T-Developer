...
class DAGWorkflowEngine:
    ...
    async def execute_workflow(self,
                              ...
                              strategy: ExecutionStrategy = ExecutionStrategy.PARALLEL) -> WorkflowExecution:
        """Execute a workflow"""
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        if node_exec.retries_left > 0:
                            ...
                            logger.info(f"Retrying node {node_id}, {node_exec.retries_left} retries left")
                        else:
                            failed.add(node_id)
    ...
    async def _execute_priority(self, execution_id: str):
        ...
        while len(completed) + len(failed) < len(dag.nodes):
            ...
            ready_nodes.sort(
                key=lambda n: dag.nodes[n].get('priority', 0),
                reverse=True
            )
    ...
    def _get_ready_nodes(self, 
        ...
        for node_id in dag.nodes:
            ...
            node_exec = execution.node_executions[node_id]
            if node_exec.status in [NodeStatus.RUNNING, NodeStatus.SUCCESS, NodeStatus.FAILED]:
                continue
            
            # Check if all dependencies are completed
            deps_completed = all(
                dep in completed for dep in dag.predecessors(node_id)
            )
            
            # Check if any dependency failed
            deps_failed = any(
                dep in failed for dep in dag.predecessors(node_id)
            )
    ...
    async def _execute_node(self, execution_id: str, node_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self._mock_agent_execution(agent_id, input_data),
                timeout=timeout
            )
    ...
    def _calculate_metrics(self, execution: WorkflowExecution) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": success_count / len(execution.node_executions) if execution.node_executions else 0
        }
