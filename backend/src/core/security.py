...
class AuthorizationManager:
    ...
    ROLE_PERMISSIONS = {
        "admin": {
            "pipeline.execute", "pipeline.view", "pipeline.delete",
            "project.create", "project.view", "project.delete",
            "user.manage", "settings.manage"
        },
        "developer": {
            "pipeline.execute", "pipeline.view",
            "project.create", "project.view", "project.delete"
        },
        "user": {
            "pipeline.execute", "pipeline.view",
        ...
        "viewer": {
            "pipeline.view", "project.view"
        }
    }
    
    @classmethod
    def check_permission(cls, context: SecurityContext, 
                        permission: str) -> bool:
    ...
    @classmethod
    def check_resource_access(cls, context: SecurityContext,
                            resource_type: str,
                            resource_id: str,
                            action: str) -> bool:
...
class RateLimiter:
    ...
    def configure_limit(self, resource: str, 
                       max_requests: int,
    ...
    def get_remaining_requests(self, resource: str, identifier: str) -> int:
        ...
        current_requests = sum(
            1 for req_time in self._request_history[key]
            if req_time > cutoff
        )
