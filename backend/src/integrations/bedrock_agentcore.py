...
class BedrockAgentCoreClient:
    ...
    def is_available(self) -> bool:
        ...
        return (
            self.bedrock_agent_runtime is not None and 
            self.config.agent_id and 
    ...
    async def invoke_agent(
        ...
        try:
            ...
            response = self.bedrock_agent_runtime.invoke_agent(
                ...
                sessionState={
                    'sessionAttributes': session.session_attributes
                }
            )
    ...
    async def _process_response_stream(self, completion_stream) -> AsyncGenerator[Dict[str, Any], None]:
        """응답 스트림 처리"""
        try:
        ...
        try:
            ...
            if not session_id:
                session_id = await self.client.create_session(
                    user_id, 
                    {"agent_name": agent_name, "framework": "t-developer"}
                )
