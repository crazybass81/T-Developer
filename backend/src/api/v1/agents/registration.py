...
async def register_agent(
    ...
    try:
        ...
        logger.error(f"Agent registration failed", error=str(e), user=current_user.username)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_agent(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Agent {agent_id} has been deprecated"
        }
...
async def analyze_agent_with_ai(
    ...
    try:
        ...
        repository = AgentRepository(db)
        await repository.update_agent(agent_id, {
        ...
        logger.error(
            f"AI analysis failed for agent {agent_id}",
            task_id=task_id,
            error=str(e)
        )
