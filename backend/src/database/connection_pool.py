...
class DatabaseConnectionPool:
    ...
    async def _load_credentials(self):
        ...
        try:
            ...
            pg_secret = await self._get_secret(f"t-developer/database/{self.environment}/master-password")
            if pg_secret:
                self.pg_config = json.loads(pg_secret)
            
            # Redis credentials
            redis_secret = await self._get_secret(f"t-developer/cache/{self.environment}/auth-token")
            if redis_secret:
    ...
    def _load_credentials_sync(self):
        ...
        try:
            # PostgreSQL credentials
            pg_secret = self._get_secret_sync(f"t-developer/database/{self.environment}/master-password")
            if pg_secret:
                self.pg_config = json.loads(pg_secret)
    ...
    def _init_dynamodb_client(self):
        """DynamoDB 클라이언트 초기화"""
        try:
            self._dynamodb_client = boto3.resource(
                'dynamodb',
