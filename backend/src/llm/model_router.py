...
class ModelRouter:
    ...
    def _filter_candidates(self, criteria: RoutingCriteria) -> List[str]:
        ...
        for model, capabilities in self.model_registry.items():
            ...
            if (criteria.target_language and 
                'all' not in capabilities.supported_languages and
                criteria.target_language not in capabilities.supported_languages):
                continue
            
            if criteria.max_latency:
                latency_order = ['low', 'medium', 'high']
                if (latency_order.index(capabilities.latency) > 
                    latency_order.index(criteria.max_latency)):
                    continue
    ...
    async def _score_models(self, candidates: List[str], criteria: RoutingCriteria) -> Dict[str, float]:
        scores = {}
