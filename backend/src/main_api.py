...
async def generate_project(request: ProjectRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not user_input or not user_input.strip():
            raise HTTPException(status_code=400, detail="프로젝트 설명을 입력해주세요")

        if len(user_input) > 2000:
            raise HTTPException(status_code=400, detail="프로젝트 설명이 너무 깁니다 (최대 2000자)")
            ...
            try:
                ...
                if pipeline_result and pipeline_result.get("success"):
                    ...
                    if download_url and download_id:
                        ...
                        if not zip_path.exists():
                            # 파일이 없으면 폴백
                            logger.warning(
                                "Download file not found, falling back to code generator"
                            )
                            download_url = None
                    ...
                    if not download_url:
                        ...
                        if generated_code and generated_code.get("files"):
                            ...
                            logger.info(
                                f"Project files created from ECS Pipeline output at {project_path}"
                            )
                    else:
                    ...
                    if CODE_GENERATOR_AVAILABLE:
                        ...
                        logger.warning(
                            f"ECS Pipeline failed: {pipeline_result.get('errors', 'Unknown error')}, falling back to template"
                        )
                        project_path = await generate_real_project(
                            project_id=project_id,
                            project_name=project_name,
                ...
                if CODE_GENERATOR_AVAILABLE:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Code Generator also failed: {gen_e}, falling back to template"
                        )
                        project_path = await generate_real_project(
                            project_id=project_id,
                            project_name=project_name,
            ...
            if CODE_GENERATOR_AVAILABLE:
                ...
                if BEDROCK_AVAILABLE:
                    try:
                        ...
                        bedrock_enhancement = (
                            await bedrock_integration.enhance_pipeline_with_bedrock(
                                {
                                    "user_input": user_input,
                                    "project_type": project_type,
                                    "features": features,
                        ...
                        logger.warning(
                            f"Bedrock enhancement failed, continuing with standard pipeline: {e}"
                        )
...
async def list_agents():
    ...
    ecs_status = (
        "ready"
        if (ECS_PIPELINE_AVAILABLE and ecs_pipeline.initialized)
        else "initializing"
        if ECS_PIPELINE_AVAILABLE
        else "unavailable"
    )
...
async def startup_event():
    ...
    if BEDROCK_AVAILABLE:
        try:
            ...
            if bedrock_success:
                ...
                logger.warning(
                    "⚠️ Bedrock AgentCore initialization failed, will run in fallback mode"
                )
        except Exception as e:
    ...
    logger.info(
        f"   • AWS Bedrock AgentCore: {'✅ Integrated' if BEDROCK_AVAILABLE else '⚠️ Unavailable'}"
    )
    logger.info(
        f"   • ECS Pipeline (9 Agents): {'✅ Ready' if ECS_PIPELINE_AVAILABLE else '⚠️ Unavailable'}"
    )
