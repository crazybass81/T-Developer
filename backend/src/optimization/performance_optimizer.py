...
class PersistentCache:
    ...
    def _evict_if_needed(self) -> None:
        ...
        entries_by_age = sorted(self.index.items(), key=lambda x: x[1]["created_at"])
...
class PerformanceOptimizer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.persistent_cache = (
            PersistentCache(max_size_mb=self.config["max_cache_size_mb"])
    ...
    async def optimize_stage_execution(
        self, stage_name: str, execution_func: Callable, input_data: Dict[str, Any]
            ...
            if execution_time > 1.0:  # Only cache expensive operations
                ...
                if self.persistent_cache:
                    self.persistent_cache.put(
                        cache_key, result, ttl_hours=self.config["cache_ttl_hours"]
                    )
    ...
    async def _execute_parallel(
        ...
        if hasattr(func, "__self__") and hasattr(func.__self__, "process_parallel"):
            return await loop.run_in_executor(
                self.process_pool, func.__self__.process_parallel, input_data
            )
        else:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if self.profiler and self.profiler.resource_history:
            ...
            report["resource_usage"] = {
                "avg_cpu_percent": sum(r.cpu_percent for r in recent_resources)
