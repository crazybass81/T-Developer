...
class AgentSquadOrchestrator:
    ...
    async def _execute_direct(
        self, user_input: str, context: PipelineContext, config: SquadConfiguration
    ) -> Dict[str, Any]:
        """직접 파이프라인 실행"""
        ...
        nl_result = await self._execute_stage(
            PipelineStage.NL_INPUT, {"query": user_input}, context, config
        )
        ...
        ui_result = await self._execute_stage(
            PipelineStage.UI_SELECTION, {"requirements": nl_result}, context, config
        )
        ...
        parsed_result = await self._execute_stage(
            ...
            {"input_data": nl_result, "ui_selection": ui_result},
            context,
            config,
        )
        ...
        component_result = await self._execute_stage(
            ...
            {"parsed_project": parsed_result, "ui_selection": ui_result},
            context,
            config,
        )
            ...
            try:
                ...
                if stage == PipelineStage.NL_INPUT:
                    ...
                    result = await agent.assemble_project(
                        input_data["generation_result"], input_data["technology_stack"]
                    )
                    result = asdict(result)

                elif stage == PipelineStage.DOWNLOAD:
                    result = await agent.prepare_download(input_data["assembly_result"])
                    result = asdict(result)
                else:
                ...
                if retries >= config.max_retries:
                    context.errors.append(
                        {"stage": stage.value, "error": str(e), "attempts": retries}
                    )
                    raise
