...
class IntegratedPipeline:
    ...
    async def execute(
        ...
        try:
            ...
            if ws_manager:
                ...
                await ws_manager.send_log(context.get('project_id'), f"✅ 프로젝트 타입: {nl_result['project_type']}", "success")
            ...
            if ws_manager:
                ...
                await ws_manager.send_log(context.get('project_id'), f"✅ UI: {ui_result['framework']}", "success")
            ...
            if ws_manager:
                ...
                await ws_manager.send_log(context.get('project_id'), f"✅ {len(component_result['components'])}개 컴포넌트 설계", "success")
            ...
            if ws_manager:
                ...
                await ws_manager.send_log(context.get('project_id'), f"✅ 매칭률: {match_result['confidence']*100:.0f}%", "success")
            ...
            if ws_manager:
                ...
                await ws_manager.send_log(context.get('project_id'), f"✅ {len(search_result['libraries'])}개 라이브러리 선택", "success")
            ...
            if ws_manager:
                ...
                await ws_manager.send_log(context.get('project_id'), f"✅ {len(generated_files)}개 파일 생성 완료", "success")
    ...
    def _parse_project_structure(self, nl_result: Dict[str, Any], ui_result: Dict[str, Any]) -> Dict[str, Any]:
        """프로젝트 구조 파싱"""
        ...
        return {
            'structure': structure,
            'entry_point': 'src/main.tsx' if 'typescript' in nl_result['features'] else 'src/main.jsx',
            'config_files': ['package.json', 'vite.config.js', 'tsconfig.json']
        }
    
    def _decide_components(self, nl_result: Dict[str, Any], ui_result: Dict[str, Any]) -> Dict[str, Any]:
        """컴포넌트 아키텍처 결정"""
    ...
    def _search_solutions(self, nl_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'libraries': libraries,
            'best_practices': ['folder-structure', 'component-patterns', 'state-management'],
