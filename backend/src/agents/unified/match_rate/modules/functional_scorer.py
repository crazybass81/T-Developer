...
class FunctionalScorer:
    ...
    async def score(
        ...
        for component in components:
            ...
            scores = {
                ...
                'integration_capabilities': self._score_integration_capabilities(capabilities, func_requirements),
                'customization_support': self._score_customization_support(capabilities, func_requirements),
                'scalability_features': self._score_scalability_features(capabilities, func_requirements)
            }
    ...
    def _score_customization_support(self, capabilities: Dict, requirements: Dict) -> float:
        ...
        customization_indicators = ['customize', 'configure', 'extend', 'plugin', 'theme', 'template']
        
        req_text = str(requirements).lower()
        cap_text = str(capabilities).lower()
        
        needs_customization = any(indicator in req_text for indicator in customization_indicators)
        supports_customization = any(indicator in cap_text for indicator in customization_indicators)
    ...
    def _score_scalability_features(self, capabilities: Dict, requirements: Dict) -> float:
        ...
        scalability_indicators = ['scale', 'performance', 'load', 'concurrent', 'cluster']
    ...
    def _calculate_coverage_percentage(self, capabilities: Dict, requirements: Dict) -> float:
        ...
        total_requirements = len(requirements.get('features', [])) + len(requirements.get('use_cases', []))
    ...
    def _analyze_critical_coverage(self, capabilities: Dict, requirements: Dict) -> Dict[str, bool]:
        ...
        critical_features = ['authentication', 'security', 'data storage', 'user management']
    ...
    def _generate_functional_recommendations(self, scores: Dict[str, float]) -> List[str]:
        ...
        for category, score in scores.items():
            if score < 0.5:
                ...
                recommendations.append(f"Moderate {category.replace('_', ' ')} - may need customization")
    ...
    def _build_functional_categories(self) -> Dict[str, List[str]]:
        ...
        return {
            'core_features': [
                ...
                'data_storage', 'search', 'reporting'
            ],
            'ui_features': [
                'responsive_design', 'mobile_support', 'accessibility',
                'themes', 'customization', 'widgets'
            ],
            'integration_features': [
                'rest_api', 'webhooks', 'third_party_integrations',
                'data_import_export', 'sso'
            ],
            'workflow_features': [
                'approval_workflows', 'notifications', 'automation',
