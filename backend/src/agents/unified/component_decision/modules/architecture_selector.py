...
class ArchitectureSelector:
    ...
    def __init__(self):
        self.architecture_criteria = {
            ArchitectureType.MICROSERVICES: {
                ...
                'drawbacks': ['Complex communication', 'Data consistency', 'Operational overhead']
            },
            ArchitectureType.SERVERLESS: {
                'indicators': ['event-driven', 'cost-sensitive', 'variable-load', 'stateless'],
                'complexity': 'medium',
                'team_size': 3,
                'benefits': ['No server management', 'Auto-scaling', 'Pay per use'],
                'drawbacks': ['Vendor lock-in', 'Cold starts', 'Limited execution time']
            },
            ArchitectureType.MONOLITHIC: {
                'indicators': ['simple', 'rapid-development', 'small-team', 'consistent'],
                'complexity': 'low',
                'team_size': 5,
                'benefits': ['Simple deployment', 'Easy debugging', 'Data consistency'],
                'drawbacks': ['Scaling limitations', 'Technology lock-in', 'Single point of failure']
            },
            ArchitectureType.EVENT_DRIVEN: {
                'indicators': ['real-time', 'asynchronous', 'reactive', 'event-sourcing'],
                'complexity': 'high',
                'team_size': 8,
                'benefits': ['Loose coupling', 'Scalability', 'Real-time processing'],
                'drawbacks': ['Complex debugging', 'Event ordering', 'Eventual consistency']
            },
            ArchitectureType.LAYERED: {
                'indicators': ['traditional', 'clear-separation', 'maintainable', 'enterprise'],
                'complexity': 'medium',
                'team_size': 6,
                'benefits': ['Clear structure', 'Separation of concerns', 'Testability'],
                'drawbacks': ['Performance overhead', 'Rigid structure', 'Cross-cutting concerns']
            }
        }
    ...
    async def select(
        ...
        if self._needs_migration(requirements):
            architecture['migration_path'] = self._generate_migration_path(
                requirements.get('current_architecture'),
                best_arch
            )
    ...
    def _score_architecture(
        ...
        if analysis['scale']['level'] == 'large':
            if arch_type in [ArchitectureType.MICROSERVICES, ArchitectureType.SERVERLESS]:
                score += 2.0
        ...
        matching_indicators = sum(
            1 for indicator in criteria['indicators']
    ...
    def _get_architecture_components(self, arch_type: ArchitectureType) -> List[str]:
        ...
        components_map = {
            ArchitectureType.MICROSERVICES: [
                ...
                'Circuit Breaker', 'Message Queue', 'Service Mesh'
            ],
            ArchitectureType.SERVERLESS: [
                'Lambda Functions', 'API Gateway', 'Event Bridge',
            ...
            ArchitectureType.EVENT_DRIVEN: [
                'Event Bus', 'Event Store', 'Event Processors',
    ...
    def _get_architecture_patterns(self, arch_type: ArchitectureType) -> List[str]:
        ...
        patterns_map = {
            ...
            ArchitectureType.SERVERLESS: [
                'Function as a Service', 'Backend for Frontend',
                'Event Sourcing', 'Choreography'
            ],
            ArchitectureType.MONOLITHIC: [
    ...
    def _get_communication_patterns(self, arch_type: ArchitectureType) -> List[str]:
        ...
        comm_map = {
            ...
            ArchitectureType.EVENT_DRIVEN: ['Pub/Sub', 'Event streaming', 'Async messaging'],
    ...
    def _build_decision_matrix(self) -> Dict:
        ...
        return {
            'factors': [
                'scalability', 'complexity', 'team_size',
                'performance', 'cost', 'time_to_market'
            ],
            'weights': {
                'scalability': 0.25,
                'complexity': 0.20,
