...
class IntegrationBuilder:
    ...
    async def _analyze_relationships(self, components: List[Component]) -> Dict[str, List[Tuple[str, ComponentRelation]]]:
    ...
    def _determine_relationship(self, comp1: Component, comp2: Component) -> Optional[ComponentRelation]:
        ...
        if any(keyword in comp1.type.lower() for keyword in ['list', 'table', 'grid']) and \
           any(keyword in comp2.type.lower() for keyword in ['detail', 'form', 'edit']):
                ...
                for integration_type in integration_types:
                    integration = Integration(
                        ...
                        configuration=self._get_integration_config(
                            integration_type, source_component, target_component, context
                        )
                    )
...
export const use{target.name.title()}Store = defineStore('{target.name.lower()}', {{
  ...
  actions: {{
    ...
    async def _build_auth_integration(
        self, integration: Integration, source: Component, target: Component, context: Dict[str, Any]
    ) -> Dict[str, str]:
