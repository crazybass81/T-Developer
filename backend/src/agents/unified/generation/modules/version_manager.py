...
class VersionManager:
    ...
    async def setup_versioning(
        ...
        try:
            ...
            version_info = await self._initialize_version_info(
            ...
            release_config = await self._setup_release_configuration(
                framework, context
            )
            ...
            await self._create_initial_changelog(
                project_path, version_info, context
            )
            ...
            return VersioningResult(
                success=False,
                version_info=VersionInfo("0.0.0", "0.0.0", VersioningStrategy.SEMANTIC, 
                                       ReleaseType.PATCH, "", "", ""),
    ...
    async def _configure_git_user(self, project_path: str):
        ...
        try:
            ...
            if not result.strip():
                ...
                await self._run_git_command(project_path, ['config', 'user.email', 'noreply@t-developer.com'])
        except:
            # Set default user
            await self._run_git_command(project_path, ['config', 'user.name', 'T-Developer'])
            await self._run_git_command(project_path, ['config', 'user.email', 'noreply@t-developer.com'])
