...
class FilterManager:
    ...
    async def _apply_category_filter(
        ...
        if 'pattern' in filter_config:
            ...
            results = [
                r for r in results
                if pattern.search(r.get('category', ''))
            ]
    ...
    async def _apply_technology_filter(
        ...
        if 'include' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('technology', '').lower() in included_techs
            ]
    ...
    async def _apply_popularity_filter(
        ...
        if 'min' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('popularity', 0) >= min_popularity
            ]
        ...
        if 'max' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('popularity', 0) <= max_popularity
            ]
        ...
        if 'github_stars' in filter_config:
            ...
            if 'min' in stars_filter:
                ...
                results = [
                    r for r in results
                    if r.get('github_stars', 0) >= min_stars
                ]
    ...
    async def _apply_quality_filter(
        ...
        if 'min' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('quality', 0) >= min_quality
            ]
        ...
        if 'max' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('quality', 0) <= max_quality
            ]
        ...
        if 'test_coverage' in filter_config:
            ...
            if 'min' in coverage_filter:
                ...
                results = [
                    r for r in results
                    if r.get('test_coverage', 0) >= min_coverage
                ]
    ...
    async def _apply_license_filter(
        ...
        if 'include' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('license', '').lower() in allowed_licenses
            ]
    ...
    async def _apply_features_filter(
        ...
        if 'preferred' in filter_config:
            ...
            for result in results:
                feature_matches = sum(
                    1 for feat in preferred_features
    ...
    async def _apply_size_range_filter(
        ...
        if 'max_size_mb' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('package_size', 0) <= max_size
            ]
        ...
        if 'bundle_size' in filter_config:
            ...
            if 'max_kb' in bundle_filter:
                ...
                results = [
                    r for r in results
                    if r.get('bundle_size', 0) <= max_bundle
                ]
    ...
    async def _apply_performance_filter(
        ...
        if 'min_score' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('performance_score', 0) >= min_perf
            ]
    ...
    async def _apply_maintenance_filter(
        ...
        if 'min_commit_frequency' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('recent_commits', 0) >= min_commits
            ]
        ...
        if 'active_maintainers' in filter_config:
            ...
            results = [
                r for r in results
                if r.get('active_maintainers', 0) >= min_maintainers
            ]
    ...
    async def _apply_security_filter(
        ...
        if 'score' in filter_config:
            ...
            if 'min' in score_filter:
                ...
                results = [
                    r for r in results
                    if r.get('security_score', 0) >= min_security
                ]
        ...
        if 'security_audit' in filter_config:
            if filter_config['security_audit']:
                results = [
                    r for r in results
                    if r.get('security_audit_passed', False)
                ]
    ...
    async def _apply_compatibility_filter(
        ...
        if 'node_version' in filter_config:
            ...
            results = [
                r for r in results
                if self._check_node_compatibility(r, node_version)
            ]
    ...
    async def _apply_custom_filter(
        ...
        if 'expression' in filter_config:
            ...
            results = [
                r for r in results
                if self._evaluate_filter_expression(r, expression)
            ]
    ...
    def _check_browser_compatibility(
        self, 
        component: Dict[str, Any], 
        browser_requirements: Dict[str, str]
    ) -> bool:
    ...
    def _check_python_compatibility(
        ...
        return any(
            python_version.startswith(v) for v in supported_versions
        )
    ...
    async def get_available_filter_options(
        ...
        options = {
            ...
            'date_range': {
                'earliest': min(
                    self._get_component_date(r, 'last_updated') 
                    for r in results
                ).isoformat(),
                'latest': max(
                    self._get_component_date(r, 'last_updated') 
