...
class SearchAnalytics:
    ...
    async def _update_realtime_stats(
        ...
        self._minute_queries = {
            k: v for k, v in self._minute_queries.items()
    ...
    def _calculate_search_metrics(self, search_event: Dict[str, Any]) -> Dict[str, float]:
        ...
        return {
            'query_complexity': search_event['query']['complexity_score'],
            'result_density': len(search_event['results']['categories']) / max(len(search_event['results']['top_scores']), 1),
            'score_consistency': 1.0 - self._calculate_score_variance_from_list(search_event['results']['top_scores']),
    ...
    def _calculate_query_specificity(self, query_text: str) -> float:
        ...
        specific_terms = [
            'framework', 'library', 'component', 'tool', 'package',
            'version', 'specific', 'exactly', 'must', 'required'
        ]
    ...
    def _calculate_technical_level(self, query_text: str) -> float:
        ...
        technical_terms = [
            ...
            'microservices', 'api', 'database', 'algorithm', 'implementation'
        ]
    ...
    def _calculate_intent_confidence(self, query: Dict[str, Any]) -> float:
        ...
        intent_indicators = ['find', 'search', 'need', 'want', 'looking for', 'help with']
    ...
    def _calculate_requirement_coverage(
        ...
        if 'category' in requirements:
            ...
            matching_results = sum(
                1 for r in results 
                if r.get('category', '').lower() == required_category
            )
        ...
        if 'technology' in requirements:
            ...
            matching_results = sum(
                1 for r in results 
                if r.get('technology', '').lower() == required_tech
            )
            coverage_score += matching_results / len(results) if results else 0
            coverage_count += 1
