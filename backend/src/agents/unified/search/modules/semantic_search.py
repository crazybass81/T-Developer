...
class SemanticSearch:
    ...
    async def _calculate_semantic_similarity(
        self, query_features: Dict[str, Any], component_features: Dict[str, Any]
    ) -> float:
        ...
        intent_score = self._calculate_intent_similarity(
            query_features["intent"], component_features["capabilities"]
        )
        ...
        concept_score = self._calculate_concept_similarity(
            query_features["concepts"], component_features["concepts"]
        )
        ...
        domain_score = self._calculate_domain_similarity(
            query_features["domain_context"], component_features["domain_info"]
        )
        ...
        embedding_score = self._calculate_embedding_similarity(
            query_features["embeddings"], component_features["embeddings"]
        )
    ...
    def _calculate_intent_similarity(
        self, query_intents: Dict[str, float], component_capabilities: List[str]
    ) -> float:
        ...
        for intent, confidence in query_intents.items():
            if intent in intent_capability_map:
                ...
                capability_match = any(
                    cap in component_capabilities for cap in required_capabilities
                )
    ...
    def _calculate_concept_similarity(
        self, query_concepts: List[str], component_concepts: List[str]
    ) -> float:
    ...
    def _calculate_domain_similarity(
        self, query_domains: Dict[str, float], component_domains: Dict[str, Any]
    ) -> float:
    ...
    def _calculate_embedding_similarity(
        self, query_embedding: List[float], component_embedding: List[float]
    ) -> float:
    ...
    def _calculate_query_specificity(self, query: str) -> float:
        ...
        specificity_score = sum(
            0.2 for indicator in specificity_indicators if indicator in query_lower
        )
    ...
    def _calculate_technical_level(self, query: str) -> float:
        ...
        query_lower = query.lower()
        tech_score = sum(0.15 for term in technical_terms if term in query_lower)
