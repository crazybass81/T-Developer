...
class ResultRanker:
    ...
    async def _rank_by_popularity(
        ...
        sorted_results = sorted(
            results, key=lambda x: x.get("popularity_rank_score", 0), reverse=True
        )
    ...
    def _extract_ml_features(
        ...
        name_matches = sum(
            1 for term in query_terms if term.lower() in result.get("name", "").lower()
        )
    ...
    def _calculate_ml_score(self, features: List[float]) -> float:
        ...
        weights = [
            ...
            0.01,
            0.01,
            0.01,  # Category features
            0.05,
            0.05,  # Numerical features
        ]
    ...
    async def get_ranking_explanation(self, result: Dict[str, Any], rank: int) -> Dict[str, Any]:
        ...
        if "ranking_details" in result:
            ...
            explanation["factors"] = {
                "relevance_contribution": details.get("relevance", 0)
                * self.ranking_weights["relevance_score"],
                "popularity_contribution": details.get("popularity", 0)
                * self.ranking_weights["popularity_score"],
                "quality_contribution": details.get("quality", 0)
                * self.ranking_weights["quality_score"],
                "recency_contribution": details.get("recency", 0)
                * self.ranking_weights["recency_score"],
                "requirements_contribution": details.get("requirements", 0)
                * self.ranking_weights["user_preference"],
                "diversity_contribution": details.get("diversity", 0)
                * self.ranking_weights["diversity"],
