...
class FacetedSearch:
    ...
    async def search(
        self, query: Dict[str, Any], components: List[Dict[str, Any]]
        ...
        for facet_type in self.facet_types.keys():
            if facet_type in self.facet_configs:
                facets = await self.facet_types[facet_type](
                    filtered_components, selected_facets.get(facet_type)
                )

                if facets:
                    facet_results[facet_type] = self._format_facets(
                        facets, facet_type, self.facet_configs[facet_type]
                    )

        return facet_results

    def _apply_facet_filters(
        self, components: List[Dict[str, Any]], selected_facets: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for facet_type, facet_values in selected_facets.items():
            if facet_type == "category" and facet_values:
                filtered = [c for c in filtered if c.get("category") in facet_values]

            elif facet_type == "technology" and facet_values:
                filtered = [c for c in filtered if c.get("technology") in facet_values]
                ...
                filtered = [c for c in filtered if c.get("license") in facet_values]
                ...
                if "min" in popularity_range and "max" in popularity_range:
                    filtered = [
                        c
                        for c in filtered
                        if popularity_range["min"]
                        <= c.get("popularity", 0)
                        <= popularity_range["max"]
                    ]
    ...
    async def _build_category_facets(
        self, components: List[Dict[str, Any]], selected: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
                ...
                facets.append(
                    {
                        ...
                        "selected": (
                            selected
                            and selected.get("min") == range_config["min"]
                            and selected.get("max") == range_config["max"]
                        ),
    ...
    async def _build_date_facets(
        self, components: List[Dict[str, Any]], selected: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def _format_facets(
        self, facets: List[Dict[str, Any]], facet_type: str, config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_tech_popularity(
        self, technology: str, components: List[Dict[str, Any]]
    ) -> float:
    ...
    async def get_available_facets(
        self, query: str, components: List[Dict[str, Any]]
    ) -> Dict[str, List[str]]:
        ...
        facets["features"] = list(
            set(feature for c in components for feature in c.get("features", []))
        )
