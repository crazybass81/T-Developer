...
class DownloadManager:
    ...
    async def get_download_stats(self) -> DownloadStats:
        ...
        bandwidth_used = sum(
            record['bytes_served'] for record in self.download_history
        )
        ...
        most_downloaded_format = max(
            format_counts.items(), 
            key=lambda x: x[1], 
            default=('unknown', 0)
        )[0] if format_counts else 'none'
    ...
    async def _check_rate_limit(self, client_ip: str) -> bool:
        ...
        recent_downloads = [
            ...
            if (record['client_ip'] == client_ip and 
                datetime.fromisoformat(record['started_at']) > one_hour_ago)
        ]
