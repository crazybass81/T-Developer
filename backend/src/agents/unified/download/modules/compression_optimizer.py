...
class CompressionOptimizer:
    ...
    async def _compress_with_method(
        ...
        output_path = os.path.join(
            target_directory,
            base_name + method_config['extension']
        )
    ...
    def _classify_file_type(self, file_path: str) -> str:
        ...
        code_extensions = {'.js', '.ts', '.py', '.java', '.cpp', '.h', '.css', '.html', '.php', '.rb', '.go', '.rs'}
    ...
    def get_compression_stats(self) -> CompressionStats:
        ...
        average_compression_ratio = sum(
            record['compression_ratio'] for record in self.optimization_history
        ) / files_processed
    ...
    async def suggest_optimal_method(
        self,
