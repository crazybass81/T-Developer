...
class UnifiedBaseAgent(Phase2BaseAgent, ABC):
    ...
    async def execute(
        ...
        if isinstance(input_data, AgentInput):
            ...
            if isinstance(context, AgentContext):
                ...
                agent_context = AgentContext(
                    trace_id=self._generate_trace_id(), metadata={"source": "ecs"}
                )

            # Create AgentInput wrapper if needed
            if isinstance(input_data, dict):
                pipeline_ctx = agent_context.pipeline_context or PipelineContext()
                agent_input = AgentInput(context=pipeline_ctx, data=input_data)
            else:
                agent_input = input_data
        ...
        try:
            ...
            self.logger.info(
                ...
                extra={"agent": self.config.name, "trace_id": agent_context.trace_id},
            )
            ...
            if hasattr(self, "process"):
                ...
                result = await asyncio.wait_for(
                    self.process(agent_input), timeout=self.config.timeout
                )
            else:
    ...
    async def _get_cached_result(
        self, input_data: Any, context: AgentContext
    ...
    async def _cache_result(
        ...
        try:
            ...
            await self.redis_client.setex(
                cache_key, self.config.cache_ttl, json.dumps(result_dict, default=str)
            )
