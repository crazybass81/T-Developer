...
async def safe_agent_execute(agent, input_data: Any) -> Dict[str, Any]:
    ...
    try:
        if isinstance(input_data, dict):
            minimal_data = {
                'query': input_data.get('data', {}).get('description', '') if 'data' in input_data else input_data.get('description', ''),
                'name': input_data.get('data', {}).get('name', 'app') if 'data' in input_data else input_data.get('name', 'app'),
                'framework': input_data.get('data', {}).get('framework', 'react') if 'data' in input_data else input_data.get('framework', 'react'),
                'features': input_data.get('data', {}).get('features', []) if 'data' in input_data else input_data.get('features', []),
            }
    ...
    if 'NLInput' in agent_name:
        return {
            'requirements': input_data.get('data', {}).get('description', '') if isinstance(input_data, dict) else '',
