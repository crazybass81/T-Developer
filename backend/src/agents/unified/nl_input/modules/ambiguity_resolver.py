...
class AmbiguityResolver:
    ...
    def _detect_in_text(self, text: str) -> List[Dict[str, Any]]:
        ...
        if 'database' in text_lower and not any(db in text_lower for db in ['postgres', 'mysql', 'mongodb', 'sqlite']):
            ambiguities.append({
        ...
        if 'api' in text_lower and not any(api in text_lower for api in ['rest', 'graphql', 'soap']):
            ambiguities.append({
                'type': 'missing_specific',
                'category': 'api',
    ...
    async def generate_questions(self, ambiguities: List[Dict[str, Any]]) -> List[str]:
        ...
        for ambiguity in ambiguities:
            ...
            if amb_type == 'ambiguous_term':
                ...
                if category == 'size':
                    questions.append(f"When you say '{term}', approximately how many items/users are we talking about?")
                elif category == 'speed':
                    questions.append(f"What specific performance metrics are you targeting (e.g., response time in ms)?")
                elif category == 'time':
