...
class UnifiedNLInputAgent(UnifiedBaseAgent):
    ...
    async def validate_input(self, input_data: AgentInput[Dict]) -> ValidationResult:
        ...
        query = input_data.data.get(
            "query",
            input_data.data.get("description", input_data.data.get("user_input", "")),
        ).strip()
    ...
    async def process(
        ...
        try:
            ...
            query = InputValidator.sanitize_string(
                data.get("query", data.get("description", data.get("user_input", ""))),
                max_length=1000,
            )
        ...
        try:
            ...
            if template_match and template_match.confidence > 0.8:
                ...
                result_data = await self._process_comprehensive(
                    enhanced_input, query, user_context, language
                )
    ...
    async def _process_comprehensive(
        ...
        tech_stack = await self.tech_analyzer.analyze(
            enhanced_input, project_type, user_context.get("preferred_stack")
        )
        ...
        ai_enhanced_data = await self._enhance_with_ai(
            enhanced_input, project_type, features, tech_stack
        )
        ai_provider = None
    ...
    async def _process_template_match(
        self, template_match: Any, enhanced_input: str, original_query: str
    ) -> EnhancedNLInputResult:
        ...
        return EnhancedNLInputResult(
            ...
            or self._extract_project_name(original_query),
            description=original_query,
...
Format as JSON."""
        ...
        if self.anthropic_client:
            try:
                return await self._process_with_claude(
                    description, {"project_type": project_type}, project_type
                )
            except Exception as e:
    ...
    async def _process_with_claude(
        self, description: str, context: Dict[str, Any], project_type: str
    ) -> Dict[str, Any]:
...
Respond in JSON format."""
        ...
        for pref_type, keywords in self.keywords["ui_preferences"].items():
            for keyword in keywords:
                if keyword in text_lower:
                    ...
                    if "style" not in preferences and pref_type not in [
                        "dark_mode",
                        "light_mode",
                    ]:
    ...
    def _estimate_effort(self, complexity: str, feature_count: int) -> int:
        ...
        base_hours = {"simple": 40, "medium": 120, "complex": 320}
    ...
    def _calculate_confidence(
        self, text: str, project_type: str, features: List[str], clarity_score: float
    ) -> float:
