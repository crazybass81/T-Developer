...
class ResponsiveDesigner:
    ...
    def design(
        ...
        layout_type: str,
        components: List[str],
        strategy: str = 'mobile_first'
    ) -> Dict[str, Any]:
        """Design responsive system"""
        
        # Select strategy
        responsive_strategy = self.layout_strategies.get(strategy, self.layout_strategies['mobile_first'])
    ...
    def _generate_media_queries(self, breakpoints: Dict, strategy: Dict) -> Dict:
        ...
        for name, config in breakpoints.items():
            if strategy['approach'] == 'min-width':
                ...
                if config['max']:
                    queries[name] = f"@media (min-width: {config['min']}px) and (max-width: {config['max']}px)"
                else:
                    queries[name] = f"@media (min-width: {config['min']}px)"
