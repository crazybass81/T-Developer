...
class DesignSystemSelector:
    ...
    def __init__(self):
        self.design_systems = {
            'material_design': {
                ...
                'version': '3.0',
                'principles': ['Material metaphor', 'Bold intentional', 'Motion meaning'],
    ...
    def select(self, requirements: Dict[str, Any]) -> Dict[str, Any]:
        ...
        customized = self._customize_system(
            selected_system,
            requirements,
            brand
        )
    ...
    def _evaluate_systems(
        ...
        for name, system in self.design_systems.items():
            ...
            # Check preferences
            if preferences.get('style') == 'professional' and name in ['material_design', 'carbon']:
                score += 0.1
    ...
    def _get_material_components(self) -> List[str]:
        return ['Button', 'TextField', 'Card', 'Dialog', 'Drawer', 'AppBar', 
                'Table', 'Tabs', 'Stepper', 'Chip', 'Avatar', 'Badge']
    
    def _get_ant_components(self) -> List[str]:
        return ['Button', 'Input', 'Card', 'Modal', 'Drawer', 'Layout',
                'Table', 'Form', 'DatePicker', 'Select', 'Tree', 'Upload']
    
    def _get_carbon_components(self) -> List[str]:
        return ['Button', 'TextInput', 'Tile', 'Modal', 'SideNav', 'Header',
                'DataTable', 'Form', 'DatePicker', 'Dropdown', 'Accordion']
    
    def _get_fluent_components(self) -> List[str]:
        return ['Button', 'TextField', 'Card', 'Dialog', 'Nav', 'CommandBar',
                'DetailsList', 'Pivot', 'Panel', 'Persona', 'MessageBar']
    
    def _get_spectrum_components(self) -> List[str]:
        return ['Button', 'TextField', 'Card', 'Dialog', 'SideBar', 'Header',
                'Table', 'Form', 'DatePicker', 'Picker', 'ActionBar']
