...
class FileOrganizer:
    ...
    def __init__(self):
        ...
        self.framework_structures = {
            'react': {
                'src/': ['components/', 'pages/', 'hooks/', 'utils/', 'services/', 'types/', 'assets/'],
            ...
            'vue': {
                'src/': ['components/', 'views/', 'composables/', 'utils/', 'services/', 'types/', 'assets/'],
            ...
            'angular': {
                ...
                'src/app/': ['components/', 'services/', 'models/', 'guards/', 'interceptors/'],
                'tests/': ['unit/', 'e2e/'],
                'docs/': ['README.md'],
                '.': ['package.json', 'angular.json', 'tsconfig.json', '.gitignore']
            },
            'express': {
                'src/': ['controllers/', 'services/', 'models/', 'middleware/', 'routes/', 'types/', 'utils/'],
            ...
            'fastapi': {
                'src/': ['routers/', 'services/', 'models/', 'schemas/', 'core/', 'utils/'],
                'tests/': ['unit/', 'integration/', 'e2e/'],
                'docs/': ['README.md', 'API.md'],
                'config/': ['settings.py', 'database.py'],
                '.': ['requirements.txt', '.gitignore', '.env.example', 'pyproject.toml']
            },
            'django': {
            ...
            'flask': {
                'src/': ['views/', 'models/', 'services/', 'utils/', 'templates/', 'static/'],
                'tests/': ['unit/', 'integration/'],
                'docs/': ['README.md'],
        ...
        self.categorization_rules = {
            FileCategory.SOURCE_CODE: {
                'extensions': ['.js', '.jsx', '.ts', '.tsx', '.py', '.java', '.go', '.rs', '.php', '.rb'],
                'patterns': ['src/', 'app/', 'lib/'],
                'exclude_patterns': ['test', 'spec', '__tests__']
            },
            FileCategory.CONFIGURATION: {
                'extensions': ['.json', '.yaml', '.yml', '.toml', '.ini', '.conf'],
                'patterns': ['config/', '.env', 'package.json', 'requirements.txt', 'Dockerfile'],
                'filenames': ['webpack.config.js', 'babel.config.js', 'jest.config.js', 'angular.json']
            },
            FileCategory.DOCUMENTATION: {
                'extensions': ['.md', '.txt', '.rst', '.adoc'],
                'patterns': ['docs/', 'README', 'CHANGELOG', 'LICENSE', 'CONTRIBUTING'],
                'filenames': ['README.md', 'CHANGELOG.md', 'LICENSE', 'CONTRIBUTING.md']
            },
            FileCategory.ASSETS: {
                'extensions': ['.png', '.jpg', '.jpeg', '.gif', '.svg', '.ico', '.css', '.scss', '.less'],
                'patterns': ['assets/', 'static/', 'public/', 'images/', 'styles/']
            },
            FileCategory.TESTS: {
                'extensions': ['.test.js', '.test.ts', '.spec.js', '.spec.ts', '.test.py'],
                'patterns': ['test/', 'tests/', '__tests__/', 'spec/', 'cypress/', 'e2e/']
            },
            ...
            FileCategory.DEPLOYMENT: {
                'patterns': ['docker', 'k8s/', 'kubernetes/', '.github/workflows/', 'deployment/'],
                'filenames': ['Dockerfile', 'docker-compose.yml', 'netlify.toml', 'vercel.json']
            },
            FileCategory.DEPENDENCIES: {
                'filenames': ['package-lock.json', 'yarn.lock', 'requirements.txt', 'Pipfile', 'go.mod', 'Cargo.toml']
            }
        }
    ...
    async def _determine_organized_path(
        ...
        if file_name in ['package.json', 'requirements.txt', '.gitignore', 'Dockerfile']:
            base_path = ''  # Root
    ...
    async def _create_organization_summary(
        ...
        for organized_file in organized_files.values():
            ...
            if category not in summary["file_categories"]:
                summary["file_categories"][category] = {
                    "count": 0,
                    "total_size": 0
                }
