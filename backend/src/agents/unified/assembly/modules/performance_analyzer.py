...
class PerformanceAnalyzer:
    ...
    def __init__(self):
        ...
        self.performance_thresholds = {
            ...
            'dependency_count': {
                'target': 20, 'warning': 50, 'critical': 100
            },
            'file_count': {
                'target': 50, 'warning': 200, 'critical': 500
            },
            'complexity_score': {
                'target': 5.0, 'warning': 10.0, 'critical': 20.0
            },
            'duplicate_code_percentage': {
                'target': 5.0, 'warning': 15.0, 'critical': 30.0
            }
        }
        ...
        self.performance_patterns = {
            'inefficient_loops': [
                ...
                (r'while\s*\([^)]*\.length[^)]*\)', 'Cache array length in while loops'),
                (r'for\s*\([^)]*in\s+.*\)', 'Use for...of instead of for...in for arrays')
            ],
            'inefficient_dom': [
                (r'document\.getElementById', 'Consider caching DOM elements'),
                (r'document\.querySelector', 'Consider caching DOM selectors'),
                (r'innerHTML\s*\+=', 'Use DocumentFragment for multiple DOM insertions')
            ],
            'memory_leaks': [
    ...
    async def analyze_performance(
        ...
        try:
            ...
            complexity_metrics = await self._analyze_code_complexity(
                validated_files
            )
            metrics.extend(complexity_metrics)
            
            # Dependency analysis
            dependency_metrics = await self._analyze_dependencies(
                validated_files
            )
            metrics.extend(dependency_metrics)
            
            # Performance patterns analysis
            pattern_metrics = await self._analyze_performance_patterns(
                validated_files
            )
            metrics.extend(pattern_metrics)
            
            # Resource optimization analysis
            resource_metrics = await self._analyze_resource_usage(
                validated_files
            )
    ...
    async def _analyze_bundle_size(
        ...
        # Analyze large files
        large_files = [(path, len(content)) for path, content in files.items() 
                      if len(content) > 10000]  # Files > 10KB
    ...
    def _get_top_recommendations(self, metrics: List[PerformanceMetric], limit: int = 5) -> List[str]:
        ...
        sorted_metrics = sorted(
            metrics,
            key=lambda m: severity_order.get(m.severity, 3)
        )
