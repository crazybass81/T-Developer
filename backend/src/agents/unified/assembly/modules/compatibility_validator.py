...
class CompatibilityValidator:
    ...
    async def validate_compatibility(
        ...
        try:
            ...
            compatibility_score = max(0, 100 - (
                critical_issues * 25 + major_issues * 10 + minor_issues * 2
            ))
            ...
            try:
                ...
                for dep, version in dependencies.items():
                    if dep == framework or dep.startswith(f'{framework}/'):
                        if not self._is_version_compatible(version, framework_config['supported_versions']):
                            issues.append(CompatibilityIssue(
                                severity='critical',
            ...
            if framework in ['express', 'fastapi']:
                for file_path, content in files.items():
                    if file_path.endswith(('.js', '.ts', '.py')):
                        if 'process.env' in content and not re.search(r'process\.env\.[A-Z_]+', content):
                            issues.append(CompatibilityIssue(
                                severity='minor',
                                component=file_path,
            ...
            for file_path, content in files.items():
                if file_path.endswith(('.js', '.jsx', '.ts', '.tsx')):
                    for api in node_apis:
                        if re.search(f"require\(['\"]?{api}['\"]?\)", content) or \
                           re.search(f"from ['\"]?{api}['\"]?", content):
                            issues.append(CompatibilityIssue(
                                severity='critical',
                                component=file_path,
