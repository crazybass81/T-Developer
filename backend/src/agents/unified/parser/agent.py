...
class UnifiedParserAgent(UnifiedBaseAgent):
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        ...
        self.entity_rules = {
            'user_management': ['user', 'authentication', 'authorization', 'role', 'permission'],
            'data_management': ['database', 'storage', 'backup', 'migration', 'schema'],
            'ui_components': ['form', 'table', 'chart', 'modal', 'navigation', 'layout'],
            'business_logic': ['workflow', 'process', 'rule', 'validation', 'calculation'],
    ...
    async def _create_acceptance_criteria(
        ...
        for story in user_stories:
            ...
            story_criteria['criteria'].append(
                f"When they perform the action"
            )
            story_criteria['criteria'].append(
                f"Then the expected outcome occurs"
            )
