...
class DataModelBuilder:
    ...
    def __init__(self):
        ...
        self.field_patterns = {
            ...
            'is_deleted': {'type': DataType.BOOLEAN, 'required': True, 'default': False}
        }
        
        # Common model templates
        self.model_templates = {
            'user': {
                'fields': ['id', 'email', 'password', 'username', 'name', 'created_at', 'updated_at'],
            ...
            'order': {
                'fields': ['id', 'user_id', 'status', 'total', 'created_at', 'updated_at'],
                'indexes': ['user_id', 'status'],
                'soft_delete': False
            },
            'category': {
                'fields': ['id', 'name', 'description', 'parent_id', 'created_at', 'updated_at'],
            ...
            if model_name in models:
                for attr in attr_list:
                    ...
                    if field_name not in models[model_name]['fields']:
                        models[model_name]['fields'][field_name] = self._infer_field_type(field_name)
    ...
    def _generate_request_body(self, model: Dict) -> Dict:
        ...
        for field_name, field_spec in model['fields'].items():
            ...
            properties[field_name] = {
                'type': field_spec['type'].value if isinstance(field_spec['type'], DataType) else field_spec['type']
            }
    ...
    def _generate_documentation(self, models: Dict[str, Dict]) -> Dict[str, str]:
        ...
        for model_name, model in models.items():
            ...
            for field_name, field_spec in model['fields'].items():
                field_type = field_spec['type'].value if isinstance(field_spec['type'], DataType) else field_spec['type']
