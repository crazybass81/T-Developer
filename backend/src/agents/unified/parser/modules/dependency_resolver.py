...
class DependencyResolver:
    ...
    def _build_dependency_graph(
        ...
        for dep in dependencies:
            if dep['type'] in [DependencyType.REQUIRES.value, DependencyType.DEPENDS_ON.value]:
    ...
    def _calculate_statistics(
        ...
        return {
            ...
            'average_dependencies': sum(len(neighbors) for neighbors in graph.values()) / len(graph) if graph else 0,
