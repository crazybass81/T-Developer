    ...
    def _extract_named_entities(
        ...
        for i, (token, tag) in enumerate(pos_tags):
            if tag == 'NNP':  # Proper noun
                ...
                context = ' '.join([t for t, _ in pos_tags[max(0, i-2):min(len(pos_tags), i+3)]])
                ...
                elif any(word in context for word in ['technology', 'framework', 'language', 'tool']):
                    entities['technologies'].append(token)
                else:
                    entities['concepts'].append(token)
        
        # Extract technology entities
        tech_keywords = ['react', 'vue', 'angular', 'python', 'javascript', 'java', 'aws', 'docker', 'kubernetes']
        for token in tokens:
