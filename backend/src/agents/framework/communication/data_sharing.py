...
class DataSharingManager:
    ...
    async def share_data(self, owner_id: str, data_type: DataType, content: Dict[str, Any], 
                        scope: ShareScope = ShareScope.AGENT_GROUP, 
                        ttl_seconds: Optional[int] = None,
                        allowed_agents: Optional[Set[str]] = None) -> str:
    ...
    async def update_shared_data(self, data_id: str, requester_id: str, 
...
class AgentDataSharingMixin:
    ...
    async def share_state(self, state: Dict[str, Any], scope: ShareScope = ShareScope.AGENT_GROUP, 
                         ttl_seconds: Optional[int] = 3600) -> str:
        """Share agent state"""
        return await self.data_manager.share_data(
            self.agent_id, DataType.STATE, state, scope, ttl_seconds
        )
    
    async def share_result(self, result: Dict[str, Any], scope: ShareScope = ShareScope.PROJECT) -> str:
