...
class RollbackManager:
    ...
    async def backup_deployment(self, deployment_id: str, version: str = "current") -> bool:
        try:
            ...
            ags_resp = await asyncio.to_thread(
                self.ba.list_agent_action_groups, agentId=self.aid, agentVersion="DRAFT"
            )
    ...
    async def _get_backup(self, deployment_id: str) -> Optional[Dict]:
        cur = self.db.cursor()
        cur.execute(
            "SELECT * FROM backups WHERE deployment_id=? ORDER BY timestamp DESC LIMIT 1",
            (deployment_id,),
        )
        row = cur.fetchone()
        if not row:
        ...
        return {
            ...
            "action_groups": json.loads(row[2]),
            "timestamp": row[3],
            "version": row[4],
        }
    ...
    async def _restore_from_backup(self, backup: Dict, rollback_id: str):
        try:
            ...
            update_params = {
                "agentId": self.aid,
                "agentName": agent_config.get("agentName"),
                "description": agent_config.get("description"),
                "instruction": agent_config.get("instruction"),
            }
            await asyncio.to_thread(
                self.ba.update_agent, **{k: v for k, v in update_params.items() if v}
            )

            current_ags = await asyncio.to_thread(
                self.ba.list_agent_action_groups, agentId=self.aid, agentVersion="DRAFT"
            )
            for ag in current_ags.get("actionGroupSummaries", []):
                try:
                    await asyncio.to_thread(
                        self.ba.delete_agent_action_group,
                        agentId=self.aid,
                        agentVersion="DRAFT",
    ...
    def get_backup_info(self, deployment_id: str) -> Optional[Dict]:
        cur = self.db.cursor()
        cur.execute(
            "SELECT deployment_id,timestamp,version FROM backups WHERE deployment_id=?",
            (deployment_id,),
        )
        row = cur.fetchone()
