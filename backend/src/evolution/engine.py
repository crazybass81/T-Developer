...
class EvolutionEngine:
            ...
            try:
                ...
                while self.current_generation < self.config.max_generations:
                    ...
                    if best_fitness >= target_fitness:
                        logger.info(f"Target fitness {target_fitness} achieved at generation {self.current_generation}")
                        break
    ...
    async def _evaluate_fitness(self, genome: Dict[str, Any]) -> float:
        ...
        memory_score = max(0, (self.config.memory_limit_kb - memory_kb) / self.config.memory_limit_kb) * 0.3
        
        # Speed score (3μs limit)
        speed_score = max(0, (self.config.instantiation_limit_us - instantiation_us) / self.config.instantiation_limit_us) * 0.2
    ...
    async def _safety_check(self) -> bool:
        ...
        if self.best_genome['metrics']['memory_kb'] > self.config.memory_limit_kb:
            logger.warning(f"Memory limit exceeded: {self.best_genome['metrics']['memory_kb']}KB > {self.config.memory_limit_kb}KB")
            return False
        
        # Check instantiation time
        if self.best_genome['metrics']['instantiation_us'] > self.config.instantiation_limit_us:
            logger.warning(f"Instantiation time exceeded: {self.best_genome['metrics']['instantiation_us']}μs > {self.config.instantiation_limit_us}μs")
            return False
    ...
    async def _save_emergency_checkpoint(self) -> None:
        ...
        checkpoint_file = self.checkpoint_dir / f"emergency_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(checkpoint_file, 'w') as f:
            json.dump(checkpoint_data, f, indent=2)
