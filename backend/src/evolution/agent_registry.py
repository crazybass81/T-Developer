...
class AgentRegistry:
    ...
    def _save_registry(self):
        ...
        data = {
            ...
            'stats': {
                ...
                'average_fitness': sum(a.fitness_score for a in self.agents.values()) / max(len(self.agents), 1)
            }
        }
    ...
    def register_agent(self, agent_path: str, name: str = None) -> Optional[str]:
        ...
        if instantiation_us > self.MAX_INSTANTIATION_US:
            print(f"⚠️ Agent instantiation slow: {instantiation_us:.2f}μs > {self.MAX_INSTANTIATION_US}μs")
        ...
        metadata = AgentMetadata(
            ...
            constraints_met=(size_kb <= self.MAX_AGENT_SIZE_KB and 
                           instantiation_us <= self.MAX_INSTANTIATION_US)
        )
