...
class AgnoEnhancedIntegration:
    ...
    def create_lightweight_agent(
        self, agent_class: Type[BaseAgent], agent_type: str, *args, **kwargs
    ) -> AgnoAgent:
        """
        경량 에이전트 생성 (Agno 최적화)
        ...
        try:
            ...
            if self.config.enable_pooling:
                if agent_type not in self.pools:
                    self.pools[agent_type] = AgentPool(max_size=self.config.pool_size)
    ...
    def optimize_agent_pipeline(self, agents: list) -> list:
        ...
        for agent in agents:
            ...
            if self._can_be_lightweight(agent_type):
                # Agno 경량 에이전트로 교체
                lightweight = self.create_lightweight_agent(agent.__class__, agent_type)
