...
class AgentRegistry:
    ...
    def __init__(self, storage_path: str = None):
        self.storage_path = Path(
            storage_path or "/home/ec2-user/T-DeveloperMVP/backend/agent_storage"
        )
    ...
    async def register_agent(
        self, agent: Any, blueprint: Dict[str, Any], generated_code: str = None
    ) -> str:
        """
        새 에이전트 등록
        ...
        agent_id = self.generate_agent_id(
            blueprint["name"], blueprint["type"], blueprint.get("version", "1.0.0")
        )
    ...
    def list_agents(
        self, type: str = None, tags: List[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
        등록된 에이전트 목록 조회
        """
    ...
    async def record_usage(
        self, agent_id: str, project_id: str, execution_time_ms: float, success: bool
    ):
        """에이전트 사용 기록"""
        conn = sqlite3.connect(str(self.db_path))
        cursor = conn.cursor()
...
async def test_registry():
    ...
    blueprint = {
        ...
        "version": "1.0.0",
        "tags": ["test", "crud", "todo"],
        "config": {"entity": "TestItem", "fields": {"id": "string", "name": "string"}},
    }
    ...
    agent_id = await agent_registry.register_agent(
        agent=agent, blueprint=blueprint, generated_code="// Test generated code"
    )
