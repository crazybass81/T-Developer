...
class SecretsManager:
    ...
    def create_or_update_secret(self, secret_id: str, secret_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            tags = [
                {'Key': k, 'Value': v} 
                for k, v in secret_data.get('tags', {}).items()
            ]
    ...
    def setup_all_secrets(self) -> Dict[str, bool]:
        ...
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'infrastructure', 'aws', 'secrets'))
        from secrets_structure import secrets_structure, staging_secrets, production_secrets
