...
class ProjectQualityValidator:
            ...
            try:
                ...
                if file_path.suffix in ['.jsx', '.tsx']:
                    ...
                    if 'import React' not in content and 'import * as React' not in content:
                        ...
                        recommendations.append(f"Consider explicit React import in {file_path.name}")
                
                # 함수 복잡도 간단 검사
                function_count = len(re.findall(r'function\s+\w+|const\s+\w+\s*=\s*\([^)]*\)\s*=>', content))
                if function_count > 10:
                    recommendations.append(f"High function count in {file_path.name}, consider splitting")
    ...
    async def _check_dependency_security(self) -> Dict[str, Any]:
        ...
        if not package_json.exists():
            return {"score": 0, "issues": ["No package.json found"], "recommendations": []}
        ...
        try:
            ...
            for dep_name, version in all_deps.items():
                if version.startswith('^') or version.startswith('~'):
                    ...
                    recommendations.append(f"Consider pinning version for {dep_name} for better security")
            ...
            if total_deps > 50:
                recommendations.append(f"High number of dependencies ({total_deps}), consider reducing")
            elif total_deps == 0:
            ...
            return {
                ...
                "vulnerable_dependencies": len([dep for dep in all_deps if dep in known_vulnerable])
            }
    ...
    async def _check_documentation(self) -> Dict[str, Any]:
        ...
        if not readme.exists():
            ...
            return {
                "score": max(0, score),
                "issues": issues,
                "recommendations": ["Add README.md with project description and setup instructions"]
            }
...
class QualityReportGenerator:
    ...
    @staticmethod
    def generate_report(metrics: QualityMetrics, project_info: Dict[str, Any] = None) -> Dict[str, Any]:
        """품질 보고서 생성"""
    ...
    def print_report(report: Dict[str, Any]):
        ...
        for category, data in report["detailed_scores"].items():
            if "score" in data:
                print(f"  {category.replace('_', ' ').title()}: {data['score']:.1f}/100 ({data.get('grade', 'N/A')})")
            elif "passed" in data:
                print(f"  {category.replace('_', ' ').title()}: {'✅ PASSED' if data['passed'] else '❌ FAILED'}")
