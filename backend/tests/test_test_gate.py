...
class TestTestConfig:
    ...
    def test_custom_config(self):
        ...
        config = TestConfig(min_coverage=90.0, min_mutation_score=70.0, enable_property_tests=False)
...
class TestTestMetrics:
    ...
    def test_metrics_passes_thresholds(self):
        ...
        config = TestConfig(min_coverage=80.0, min_mutation_score=60.0)

        good_metrics = TestMetrics(line_coverage=85.0, branch_coverage=80.0, mutation_score=65.0)
...
"""
        ...
        from packages.evaluation.test_gate import MutationResult, TestMetrics

        mock_metrics = TestMetrics(line_coverage=85.0, branch_coverage=80.0, uncovered_lines=[])
        gate.coverage_analyzer = AsyncMock()
        gate.coverage_analyzer.analyze_coverage.return_value = mock_metrics

        mock_mutation = MutationResult(mutation_score=70.0, total_mutants=10, killed_mutants=7)
...
"""
    ...
    async def test_generate_report(self, gate):
        ...
        result = TestResult(
            ...
            survived_mutants=[{"file": "src/main.py", "line": 10, "mutation": "Changed + to -"}],
        )
    ...
    def test_should_fail_build(self, gate):
        ...
        result_low_coverage = TestResult(
            passed=False, metrics=TestMetrics(line_coverage=70.0)  # Below 80% threshold
        )
        ...
        result_low_mutation = TestResult(
            passed=False,
            metrics=TestMetrics(line_coverage=85.0, mutation_score=50.0),  # Below 60% threshold
        )
        assert gate.should_fail_build(result_low_mutation) is True

        # Should pass with good metrics
        result_good = TestResult(
            passed=True, metrics=TestMetrics(line_coverage=85.0, mutation_score=65.0)
        )
