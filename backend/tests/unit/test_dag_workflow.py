...
class TestDAGWorkflow:
    ...
    async def test_conditional_execution(self, engine):
        ...
        execution = await engine.execute_workflow(
            workflow_id="conditional_test",
            input_data={"should_execute": False}
        )
    ...
    def test_execution_status_tracking(self, engine):
        ...
        execution = loop.run_until_complete(
            engine.execute_workflow("status_test")
        )
...
def run_tests():
    ...
    print(f"   Success: {execution.metrics['success_count']}, Failed: {execution.metrics['failed_count']}")
