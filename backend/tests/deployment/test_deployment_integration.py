...
from src.deployment.rollback_manager import RollbackManager, RollbackReason, RollbackSpec
...
class TestDeploymentSystemIntegration:
    ...
    def mock_aws_services(self):
        ...
        with patch("boto3.client") as mock_client:
            ...
            mock_bedrock_agent.list_agent_action_groups.return_value = {"actionGroupSummaries": []}
            mock_bedrock_runtime.invoke_agent.return_value = {"response": "Test response"}
    ...
    async def test_deployment_failure_with_tracking(self, temp_dbs, mock_aws_services):
        ...
        with patch.dict(os.environ, {"BEDROCK_AGENT_ID": "test_agent_id"}):
            ...
            spec = AgentSpec(name="failing_agent", code="def execute(): pass", version="1.0.0")

            # Start tracking
            await tracker.start_tracking("failing_deploy_123", spec.name, spec.version)
            await tracker.update_status(
                "failing_deploy_123", DeployStatus.BUILDING, "Starting build"
            )
    ...
    async def test_deployment_rollback_integration(self, temp_dbs, mock_aws_services):
        ...
        with patch.dict(os.environ, {"BEDROCK_AGENT_ID": "test_agent_id"}):
            ...
            rollback_manager.db = rollback_manager.__class__().__dict__["_init_backup_store"](
                rollback_manager
            )
            ...
            await tracker.start_tracking(
                "rollback_operation_123", "rollback_to_v1", "rollback_1.0.0"
            )
            await tracker.update_status(
    ...
    async def test_concurrent_deployments_with_tracking(self, temp_dbs, mock_aws_services):
        ...
        with patch.dict(os.environ, {"BEDROCK_AGENT_ID": "test_agent_id"}):
            ...
            for i, spec in enumerate(specs):
                ...
                await tracker.update_status(
                    deployment_id, DeployStatus.BUILDING, f"Building agent {i}"
                )
            ...
            for i, result in enumerate(deployment_results):
                ...
                if isinstance(result, Exception):
                    ...
                    await tracker.update_status(
                        deployment_id, DeployStatus.DEPLOYED, f"Agent {i} deployed: {result['id']}"
                    )
    ...
    async def test_system_resilience_to_failures(self, temp_dbs, mock_aws_services):
        ...
        with patch.dict(os.environ, {"BEDROCK_AGENT_ID": "test_agent_id"}):
            ...
            await tracker.update_status(
                "resilience_test_123", DeployStatus.FAILED, "First attempt failed"
            )
            ...
            await tracker.update_status(
                "resilience_retry_123", DeployStatus.DEPLOYED, "Retry succeeded"
            )
...
class TestPerformanceIntegration:
    ...
    async def test_deployment_system_performance(self):
        ...
        with patch("boto3.client") as mock_client, patch.dict(
            os.environ, {"BEDROCK_AGENT_ID": "test_agent_id"}
        ):
            ...
            try:
                ...
                await tracker.update_status(
                    "perf_test_123", DeployStatus.DEPLOYED, "Performance test complete"
                )
