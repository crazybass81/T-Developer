...
class TestDeploymentTracker:
    ...
    def test_get_deployment_from_db(self, tracker):
        ...
        conn.execute(
            ...
            ("dep456", "db_agent", "1.0.0", "deployed", time.time(), time.time(), None),
        )
        conn.execute(
    ...
    def test_get_active_deployments(self, tracker):
        ...
        tracker.active_tracks["active1"] = DeployRecord(
            "active1", "test", "1.0", DeployStatus.BUILDING, time.time()
        )
        tracker.active_tracks["active2"] = DeployRecord(
            "active2", "test", "1.0", DeployStatus.DEPLOYING, time.time()
        )
        tracker.active_tracks["completed"] = DeployRecord(
            "completed", "test", "1.0", DeployStatus.DEPLOYED, time.time()
        )
        tracker.active_tracks["failed"] = DeployRecord(
            "failed", "test", "1.0", DeployStatus.FAILED, time.time()
        )
    ...
    async def test_full_tracking_lifecycle(self):
        ...
        try:
            ...
            await tracker.update_status(
                "lifecycle_test", DeployStatus.DEPLOYING, "Deploying to AgentCore"
            )
            await tracker.update_status(
                "lifecycle_test", DeployStatus.DEPLOYED, "Successfully deployed"
            )
