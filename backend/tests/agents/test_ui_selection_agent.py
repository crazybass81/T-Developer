...
from src.lambda.agents.ui_selection_agent import (
    UISelectionAgent,
    FrameworkCategory,
    FrameworkInfo,
    UIStack,
...
class TestUISelectionAgent:
    ...
    def agent(self):
        ...
        with patch('src.lambda.agents.ui_selection_agent.ssm') as mock_ssm:
            mock_ssm.get_parameters_by_path.return_value = {
                'Parameters': [
                    {'Name': '/t-developer/test/ui-selection-agent/max_alternatives', 'Value': '3'},
                    {'Name': '/t-developer/test/ui-selection-agent/min_confidence_score', 'Value': '0.5'}
                ]
            }
    ...
    def test_select_ui_stack_mobile(self, agent):
        ...
        assert result.recommended_stack.framework in ['react-native', 'flutter', 'ionic']
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--cov=src.lambda.agents.ui_selection_agent", "--cov-report=term-missing"])
