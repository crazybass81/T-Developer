...
class TestPipelineIntegration:
    ...
    async def test_parallel_agent_execution(self, agno_orchestrator):
        ...
        results = await agno_orchestrator.execute_parallel(
            [a.name for a in analysis_agents],
            {"query": "test"}
        )
    ...
    async def test_caching_mechanism(self, agno_orchestrator):
        ...
        result1 = await agno_orchestrator.execute_parallel(
            ["cached_agent"],
            input_data
        )
        
        result2 = await agno_orchestrator.execute_parallel(
            ["cached_agent"],
            input_data
        )
