...
class TestTenant:
    ...
    def test_tenant_creation(self):
        ...
        tenant = Tenant(id="test-123", name="Test Company", tier=TenantTier.STARTER)
...
class TestResourceQuota:
    ...
    def test_quota_creation(self):
        ...
        quota = ResourceQuota(
            resource_type=ResourceType.CPU_CORES, limit=8.0, warning_threshold=0.8
        )
...
class TestTenantManager:
    ...
    async def test_create_tenant(self, tenant_manager):
        ...
        tenant = await tenant_manager.create_tenant(
            "Test Company", TenantTier.PROFESSIONAL, "admin@test.com"
        )
    ...
    async def test_create_duplicate_tenant(self, tenant_manager):
        ...
        # Create first tenant
        await tenant_manager.create_tenant("Test Company", TenantTier.STARTER, "admin@test.com")

        # Try to create duplicate
        with pytest.raises(ValueError, match="already exists"):
            await tenant_manager.create_tenant(
                "Test Company", TenantTier.STARTER, "admin2@test.com"
            )
    ...
    async def test_invalid_tenant_name(self, tenant_manager):
        ...
        with pytest.raises(ValueError, match="Invalid tenant name"):
            await tenant_manager.create_tenant(
                "", TenantTier.STARTER, "admin@test.com"  # Empty name
            )

        with pytest.raises(ValueError, match="Invalid tenant name"):
            await tenant_manager.create_tenant(
                "A", TenantTier.STARTER, "admin@test.com"  # Too short
            )
    ...
    async def test_activate_tenant(self, tenant_manager):
        ...
        # Create tenant
        tenant = await tenant_manager.create_tenant(
            "Test Company", TenantTier.STARTER, "admin@test.com"
        )
...
class TestTenantManagerIntegration:
    ...
    async def test_full_tenant_lifecycle(self):
        ...
        tenant = await manager.create_tenant(
            "Lifecycle Test Corp", TenantTier.PROFESSIONAL, "admin@lifecycle.com"
        )
    ...
    async def test_concurrent_tenant_operations(self):
        ...
        for i in range(5):
            task = manager.create_tenant(
                f"Concurrent Corp {i}", TenantTier.STARTER, f"admin{i}@concurrent.com"
            )
            tasks.append(task)
        ...
        activation_tasks = [manager.activate_tenant(tenant.id) for tenant in tenants]
    ...
    async def test_tenant_quota_enforcement_integration(self):
        ...
        enterprise_tenant = await manager.create_tenant(
            "Enterprise Corp", TenantTier.ENTERPRISE, "admin@enterprise.com"
        )

        # Create starter tenant (no auto-scaling)
        starter_tenant = await manager.create_tenant(
            "Starter Corp", TenantTier.STARTER, "admin@starter.com"
        )
