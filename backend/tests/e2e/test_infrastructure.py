        ...
        try:
            ...
            if response['repositories']:
                ...
                images_response = ecr.list_images(
                    repositoryName='t-developer-backend',
                    maxResults=5
                )
...
def run_e2e_tests():
    ...
    # Run tests
    tests = [
        ('AWS Secrets Access', lambda: test_instance.test_aws_secrets_access(aws_secrets)),
        ('AWS Parameters Access', lambda: test_instance.test_aws_parameters_access(aws_parameters)),
        ('OpenAI Connection', lambda: test_instance.test_openai_connection(aws_secrets)),
        ('Anthropic Connection', lambda: test_instance.test_anthropic_connection(aws_secrets)),
        ('Database Connection', lambda: test_instance.test_database_connection(aws_secrets)),
        ('Redis Connection', lambda: test_instance.test_redis_connection(aws_parameters)),
        ('S3 Access', lambda: test_instance.test_s3_access(aws_clients, aws_parameters)),
        ('CloudWatch Metrics', lambda: test_instance.test_cloudwatch_metrics(aws_clients)),
        ('ECS Cluster Status', lambda: test_instance.test_ecs_cluster_status(aws_clients, aws_config)),
        ('ECR Repository', lambda: test_instance.test_ecr_repository_exists(aws_clients)),
        ('End-to-End Flow', lambda: test_instance.test_end_to_end_flow(aws_secrets, aws_parameters))
    ]
