...
class SecretScanner:
    ...
    def scan_s3_object(self, bucket: str, key: str) -> Dict[str, Any]:
        """S3 객체 스캔"""
        source_info = {"type": "s3", "bucket": bucket, "key": key, "size": 0}
        ...
        try:
            ...
            return {"source": source_info, "findings": [], "error": str(e)}
...
def send_alert(scan_result: Dict[str, Any]) -> None:
    ...
    if summary["critical"] > 0:
        ...
        # 중간 위험도 비밀 - 로그만
        logger.info(
            f"Medium severity secrets detected in {source['bucket']}/{source['key']}: {summary}"
        )
...
def lambda_handler(event, context):
    ...
    try:
        ...
        return {
            "statusCode": 500,
            "body": {"error": str(e), "timestamp": datetime.utcnow().isoformat()},
        }
...
if __name__ == "__main__":
    ...
    result = lambda_handler(
        test_event,
        type("Context", (), {"get_remaining_time_in_millis": lambda: 300000})(),
    )
