...
class RuleChecker:
    ...
    def check_production_ready(self) -> None:
        ...
        # Validation 체크
        validation_count = self._count_pattern(
            r"(validate|Validator|validation)", [".py", ".ts", ".tsx"]
        )
    ...
    def check_language_rules(self) -> None:
        ...
        if ts_agents > py_agents:
            self.warnings.append(
                f"⚠️  TypeScript Agent가 Python보다 많음 (TS: {ts_agents}, PY: {py_agents})"
            )
    ...
    def _search_pattern(self, pattern: str, extensions: List[str]) -> List[str]:
        ...
        for ext in extensions:
            for file_path in self.project_root.rglob(f"*{ext}"):
                ...
                if (
                    "test" in str(file_path) or "spec" in str(file_path)
                ) and "mock" in pattern.lower():
