...
def check_commit_size() -> bool:
    ...
    if len(changed_files) > CONSTRAINTS["max_files_per_commit"]:
        print(f"❌ Too many files changed: {len(changed_files)} > {CONSTRAINTS['max_files_per_commit']}")
        ...
        if result.stdout:
            ...
            if len(parts) >= 2:
                ...
                if total_changes > CONSTRAINTS["max_changes_per_file"]:
                    print(f"❌ Too many changes in {file}: {total_changes} > {CONSTRAINTS['max_changes_per_file']}")
                    return False
