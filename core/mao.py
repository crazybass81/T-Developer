...
class MAO:
    ...
    def _generate_pr_description(self, task: Task) -> str:
        ...
        """
        # 계획 요약 가져오기
        plan_summary = task.plan_summary or "계획 요약 없음"
        
        # 수용 기준 추출 (계획에서 가져오기)
        acceptance_criteria = "기본 수용 기준"
        try:
            if task.plan_s3_key:
                ...
                if "acceptance_criteria" in plan_json and isinstance(plan_json["acceptance_criteria"], list):
                    criteria_list = plan_json["acceptance_criteria"]
                    acceptance_criteria = "\n".join([f"- {item}" for item in criteria_list])
        except Exception as e:
            logger.warning(f"Failed to extract acceptance criteria from plan: {e}")
        
        # 파일 목록 포맷팅
        modified_files_list = "\n".join([f"- {file}" for file in task.modified_files]) if task.modified_files else "None"
        created_files_list = "\n".join([f"- {file}" for file in task.created_files]) if task.created_files else "None"
        
        # 테스트 결과 포맷팅
        test_result = "✅ All tests passed" if task.test_success else "❌ Some tests failed"
