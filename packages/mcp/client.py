...
from packages.mcp.server.main import ToolRequest, ToolResponse, ToolType
...
class MCPClient:
    ...
    def __init__(self, server_config: Optional[str] = None):
        ...
        """
        self.server_config = server_config or str(
            Path(__file__).parent / "server" / "config.json"
        )
    ...
    async def git_commit(self, message: str, files: Optional[list] = None) -> str:
        ...
        """
        # Add files
        add_response = await self.send_request(
            ToolType.GIT,
            "add",
            {"files": files or ["."]}
        )
        ...
        # Commit
        commit_response = await self.send_request(
            ToolType.GIT,
            "commit",
            {"message": message}
        )
