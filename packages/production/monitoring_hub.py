...
class AlertRule:
    ...
    def should_fire(self, current_value: float, duration_met: bool) -> bool:
        ...
        if (self.last_fired and 
            (datetime.utcnow() - self.last_fired).total_seconds() < self.cooldown_seconds):
            return False
...
class MonitoringHub:
    ...
    async def _calculate_availability_slo(self, slo: SLO) -> None:
        ...
        slo.error_budget_consumed = max(0.0, 100.0 - availability) / (100.0 - slo.target_percentage) * 100.0
    ...
    async def _calculate_latency_slo(self, slo: SLO) -> None:
        ...
        if p95_latency <= target_latency:
            ...
            slo.compliance_percentage = max(0.0, slo.target_percentage * (target_latency / p95_latency))
