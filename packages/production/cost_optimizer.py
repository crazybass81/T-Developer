...
class ResourceUsageMetrics:
    ...
    def waste_cost_per_hour(self) -> float:
        ...
        if self.is_underutilized:
            ...
            unused_percentage = (100 - max(self.avg_cpu_utilization, self.avg_memory_utilization)) / 100
...
class CostOptimizer:
        ...
        for days_ago in range(30):
            ...
            for tenant_id in sample_tenants:
                for resource_id in sample_resources:
                    ...
                    base_cost = {"web-server": 2.40, "db-server": 4.80, "storage": 0.50, "load-balancer": 0.60}
                    resource_type_key = resource_id.split("-")[0]
                    
                    if resource_type_key in base_cost:
                        daily_cost = base_cost[resource_type_key] * (0.8 + 0.4 * (days_ago % 7) / 7)  # Weekly variation
                        
                        record = CostRecord(
                            record_id=f"cost_{tenant_id}_{resource_id}_{timestamp.date()}",
                            timestamp=timestamp,
                            tenant_id=tenant_id,
                            resource_type=ResourceType.COMPUTE if "server" in resource_id else ResourceType.STORAGE,
    ...
    async def record_cost(self, tenant_id: str, resource_type: ResourceType,
                         resource_id: str, cost_amount: float,
                         usage_quantity: float = 0.0, usage_unit: str = "",
                         region: str = "", tags: Dict[str, str] = None) -> CostRecord:
        """Record a cost entry.
    ...
    async def _analyze_cost_trends(self) -> None:
        ...
        now = datetime.utcnow()
        last_7_days = [record for record in self._cost_records 
        ...
        for budget in self._budgets.values():
            ...
            for threshold in budget.alerts:
                if utilization >= threshold:
                    ...
                    recent_alerts = [
                        alert for alert in self._cost_alerts
                        if (alert.get("budget_id") == budget.budget_id and
                            alert.get("threshold") == threshold and
                            (datetime.utcnow() - datetime.fromisoformat(alert["timestamp"])).hours < 6)
                    ]
                    
                    if not recent_alerts:
                        severity = "critical" if threshold >= 90 else ("warning" if threshold >= 75 else "info")
    ...
    async def update_resource_metrics(self, resource_id: str, resource_type: ResourceType,
                                    tenant_id: str, cpu_utilization: float = 0.0,
                                    memory_utilization: float = 0.0,
                                    storage_utilization: float = 0.0,
                                    idle_time_percentage: float = 0.0) -> None:
        ...
        metrics.avg_cpu_utilization = (alpha * cpu_utilization + 
                                     (1 - alpha) * metrics.avg_cpu_utilization)
        metrics.avg_memory_utilization = (alpha * memory_utilization + 
                                        (1 - alpha) * metrics.avg_memory_utilization)
        metrics.avg_storage_utilization = (alpha * storage_utilization + 
                                         (1 - alpha) * metrics.avg_storage_utilization)
        metrics.idle_time_percentage = (alpha * idle_time_percentage + 
                                      (1 - alpha) * metrics.idle_time_percentage)
        ...
        for resource_id, metrics in self._resource_metrics.items():
            if metrics.is_underutilized and metrics.cost_per_hour > 0:
                ...
                if potential_savings_monthly > 50:  # Minimum threshold for recommendation
                    recommendation_id = f"rightsize_{resource_id}_{int(datetime.utcnow().timestamp())}"
    ...
    async def _analyze_storage_optimization(self) -> None:
        ...
        for record in self._cost_records:
            if (record.resource_type == ResourceType.STORAGE and 
                record.timestamp >= last_30_days):
        ...
        if len(regional_costs) > 1:
            ...
            if cost_difference > 1000:  # $1000+ difference
                ...
                recommendation = OptimizationRecommendation(
                    ...
                    title=f"Consider regional optimization",
                ...
                self._recommendations[recommendation_id] = recommendation
                self.logger.info(f"Generated regional optimization recommendation: {recommendation_id}")
        ...
        for resource_id, metrics in self._resource_metrics.items():
            if metrics.resource_type == ResourceType.COMPUTE and metrics.cost_per_hour > 1.0:
                ...
                if potential_savings > 100:  # Minimum threshold
                    recommendation_id = f"schedule_{resource_id}_{int(datetime.utcnow().timestamp())}"
                    ...
                    self._recommendations[recommendation_id] = recommendation
                    self.logger.info(f"Generated schedule-based optimization recommendation: {recommendation_id}")
    ...
    async def _cleanup_old_data(self) -> None:
        ...
        if cleaned_records > 0 or expired_recommendations:
            self.logger.info(f"Cleaned up {cleaned_records} old cost records and {len(expired_recommendations)} expired recommendations")
            
    async def create_budget(self, name: str, amount: float, period: str = "monthly",
                          tenant_id: Optional[str] = None,
                          categories: Set[CostCategory] = None) -> Budget:
        """Create a new budget.
    ...
    async def get_cost_summary(self, tenant_id: Optional[str] = None,
        ...
        records = [
            ...
            if (record.timestamp >= cutoff_date and
                (tenant_id is None or record.tenant_id == tenant_id))
        ]
        ...
        return {
            ...
            "by_day": dict(by_day),
            "top_resources": [
                {"resource_id": resource_id, "cost": cost}
                for resource_id, cost in top_resources
            ]
        }
    ...
    async def get_cost_status(self) -> Dict[str, Any]:
        ...
        underutilized_resources = sum(1 for m in self._resource_metrics.values() if m.is_underutilized)
        overutilized_resources = sum(1 for m in self._resource_metrics.values() if m.is_overutilized)
        ...
        return {
            ...
            "budgets": {
                ...
                "overall_utilization": (total_budget_spent / total_budget_amount * 100) if total_budget_amount > 0 else 0,
            ...
            "alerts": {
                "recent_count": len([a for a in self._cost_alerts if (datetime.utcnow() - datetime.fromisoformat(a["timestamp"])).days <= 1]),
                "total_count": len(self._cost_alerts)
            },
            "last_updated": datetime.utcnow().isoformat()
        }
        
    def get_recommendations(self, tenant_id: Optional[str] = None,
                          optimization_type: Optional[OptimizationType] = None) -> List[Dict[str, Any]]:
