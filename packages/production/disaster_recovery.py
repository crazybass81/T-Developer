...
class DisasterRecovery:
        ...
        for target in self._backup_targets.values():
            if self._is_backup_due(target, now):
                try:
                    ...
                    self.logger.error(f"Failed to start scheduled backup for {target.target_id}: {e}")
        ...
        for target in self._backup_targets.values():
            if target.last_backup:
                ...
                if days_since_backup > 2:  # More than 2 days
                    self.logger.warning(f"Target {target.target_id} has not been backed up for {days_since_backup} days")
                    
    async def add_backup_target(self, name: str, source_type: DataSource,
                              source_path: str, schedule: str = "0 2 * * *",
                              retention_days: int = 30) -> BackupTarget:
    ...
    async def start_backup(self, target_id: str, 
...
"""
    ...
    async def _backup_filesystem(self, target: BackupTarget, job: BackupJob) -> Dict[str, Any]:
        ...
        # Create mock backup content
        backup_content = f"Filesystem backup of {target.source_path}\nCreated: {datetime.utcnow()}\n"
        backup_file.write_text(backup_content)
        ...
        return {
            ...
            "metadata": {
                ...
                "file_count": 1  # Mock file count
            }
        }
        
    async def _backup_application_state(self, target: BackupTarget, job: BackupJob) -> Dict[str, Any]:
...
"""
    ...
    async def start_recovery(self, plan_id: str, recovery_type: RecoveryType,
                           target_time: Optional[datetime] = None) -> RecoveryOperation:
    ...
    async def _execute_recovery(self, operation: RecoveryOperation, 
        ...
        try:
            ...
            self.logger.info(f"Recovery completed: {operation.operation_id} (success rate: {success_rate:.1%})")
    ...
    async def _recover_target(self, target_id: str, operation: RecoveryOperation) -> None:
        ...
        target_jobs = [
            ...
            if (job.target_id == target_id and 
                job.is_completed and
                (not operation.target_point_in_time or 
                 job.completed_at <= operation.target_point_in_time))
        ]
    ...
    async def test_recovery_plan(self, plan_id: str) -> Dict[str, Any]:
        ...
        for prereq in plan.prerequisites:
            ...
            test_results["prerequisites_met"].append({
                "prerequisite": prereq,
                "status": "met"  # Simulate all prerequisites met
            })
        ...
        for target_id in plan.recovery_targets:
            ...
            if target_jobs:
                ...
                test_results["backup_availability"][target_id] = {
                    ...
                    "backup_age_hours": (datetime.utcnow() - latest_backup.completed_at).total_seconds() / 3600
                }
            else:
                test_results["backup_availability"][target_id] = {
                    "available": False,
    ...
    async def get_dr_status(self) -> Dict[str, Any]:
        ...
        failed_jobs = sum(1 for job in self._backup_jobs.values() if job.status == BackupStatus.FAILED)
        ...
        total_backup_size = sum(
            job.size_bytes for job in self._backup_jobs.values() 
            if job.is_completed
        )
        ...
        return {
            ...
            "backup_targets": {
                ...
                "with_recent_backups": sum(
                    1 for target in self._backup_targets.values()
            ...
            "recovery_plans": {
                ...
                "tested_recently": sum(
                    1 for plan in self._recovery_plans.values()
                    if plan.last_tested and (datetime.utcnow() - plan.last_tested).days <= 90
                ),
                "avg_rto_minutes": sum(plan.estimated_rto_minutes for plan in self._recovery_plans.values()) / len(self._recovery_plans) if self._recovery_plans else 0
            },
            "recovery_operations": {
                "recent": len(recent_recoveries),
                "success_rate": (
                    sum(1 for op in recent_recoveries if op.success_rate > 0.8) / len(recent_recoveries) * 100
    ...
    def list_backup_jobs(self, target_id: Optional[str] = None,
