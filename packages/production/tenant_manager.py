...
class Tenant:
    ...
    metadata: TenantMetadata = field(default_factory=lambda: TenantMetadata(
    ...
    def add_quota(self, resource_type: ResourceType, limit: float, 
                  auto_scale: bool = False) -> None:
    ...
    def check_quota(self, resource_type: ResourceType, 
...
class TenantManager:
    ...
    async def terminate_tenant(self, tenant_id: str, 
        ...
        try:
            ...
            self.logger.info(
                f"Terminated tenant: {tenant_id} (preserve_data={preserve_data})"
            )
            return True
    ...
    async def _throttle_tenant_resource(self, tenant: Tenant, 
    ...
    async def get_tenant_metrics(self, tenant_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "resource_usage": {
                rt.value: quota.usage_percentage 
                for rt, quota in tenant.quotas.items()
            },
            "quota_violations": [
                rt.value for rt in tenant.is_quota_exceeded()
            ],
